...
- **CLI 업데이트:** `has_update && !disabled && !is_downgrade` 이고 halt marker 의 버전이 `latest` 와 다르면 `axhub update apply --execute --yes --json` 을 실행해요(즉시 반영, 판정은 exit 코드). exit 14/66(digest·cosign) → `~/.axhub/cache/.auto-update-halt`(`<latest>|<exit>`)를 쓰고 같은 버전은 재시도하지 않아요. 새 `latest` 가 나오면 halt 를 지우고 다시 시도해요.
