...
This project is indexed by GitNexus as **axhub** (1036 symbols, 1182 relationships, 10 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
auto-update 와 나란히 SessionStart 훅이 하나 더 있어요 (`hooks/hooks.json` 두 번째 entry). 새로 등록한 MCP 서버는 Claude Code 를 재시작해야 세션에 로드되기 때문에, onboarding 은 `claude mcp add` 직후 marker(`~/.axhub/cache/.onboarding-mcp-restart`)를 쓰고 Restart Handoff Card 로 종료해요. 재시작 후 이 훅이 marker(7일 TTL, mtime 만 사용)를 감지하면 새 세션이 온보딩 마무리(`claude mcp get axhub` 확인 → 필요시 host-aware OAuth → 최종 카드 → marker 삭제)를 다시 묻지 않고 바로 이어가요 — 사용자의 첫 요청이 다른 주제면 한 줄 제안만 남기고 그 요청을 우선해요. 일반 Claude Code 는 `/mcp` 를 쓰지만 Claude Desktop Code mode 채팅은 slash command 를 실행하지 않으므로, Desktop 에서는 `Cmd/Ctrl+J` 내장 터미널에서 별도 대화형 Claude Code 를 열어 인증하도록 안내해 deadlock 을 피해요.
