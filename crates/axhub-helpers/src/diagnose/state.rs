...
impl DiagnoseState {
    ...
    pub fn transition(self, event: DiagnoseEvent) -> Result<Self, DiagnoseError> {
        ...
        let next = match (self, event) {
            ...
            (src, ev) => {
                return Err(DiagnoseError::FixApplyFailed(format!(
    ...
    pub fn is_terminal(self) -> bool {
        matches!(self, DiagnoseState::Idle)
    }
}
...
impl DiagnoseSession {
    ...
    pub fn snapshot(&self) -> DiagnoseState {
        *self.state.lock().expect("diagnose state mutex poisoned")
    }
    ...
    pub fn apply(&self, event: DiagnoseEvent) -> Result<DiagnoseState, DiagnoseError> {
        let mut guard = self.state.lock().expect("diagnose state mutex poisoned");
