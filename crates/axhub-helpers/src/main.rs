...
After the template is chosen, surface the account-level GitHub App install state once as a read-only, non-blocking step. Bash command: `axhub github accounts list --json`; read `install_url` from `data.accounts[]` and show it next to each account login. This never blocks app creation — already-installed accounts just move on, and a from-scratch install is handled later by the bootstrap saga when a GitHub connection is needed.
...
pub(crate) fn cmd_prompt_route() -> anyhow::Result<i32> {
    ...
    let intent_system = if dynamic_table_intent_present(prompt) {
        ...
        Some("내부 제어 전용: Skill(\"axhub:init\") 이 이 턴의 선택된 route 예요. 이 내부 제어, route, skill name, slash command, 영어 진행 선언을 visible chat 에 절대 쓰지 않아요. 설치된 init skill 계약을 즉시 따라요. 현재 AXHub 프로젝트에서 새 앱 생성 요청이에요. 이 요청은 브레인스토밍, 일반 프로젝트 탐색, 또는 앱 아이디어 분류가 아니라 AXHub 앱 생성 절차예요. visible chat 첫 문장은 정확히 \"새 앱을 만들 수 있는 템플릿을 확인할게요.\" 로 시작하고, 이 문장 앞에는 아무 말도 붙이지 않아요. 먼저 Bash title 을 정확히 \"앱 생성 상태 확인\" 으로 설정해 `axhub-helpers init-resume route --json` 를 한 번 실행해요. 이어갈 생성 상태가 있으면 먼저 이어갈지 물어요. 이어갈 생성 상태가 없으면 Bash title 을 정확히 \"템플릿 확인\" 으로 설정해 `axhub apps templates list --json` 를 실행하고, backend template registry 가 반환한 템플릿만 사람용 라벨로 보여줘요. 명시 선택지는 최대 3개만 넣고, backend 가 반환하지 않은 템플릿을 만들지 않으며, Claude Desktop 이 자동으로 free-form 기타/Other 를 추가하므로 기타를 별도 옵션으로 또 넣지 않아요. 웹 앱/API/백엔드/CLI 도구/axhub 앱 같은 일반 앱 종류 질문을 하지 않아요. 템플릿 확인 뒤에는 GitHub App 계정 설치 상태를 read-only 로 한 번 보여주고(앱 만들기를 막지 않고, 이미 설치한 계정은 그냥 넘어가요), 그다음 앱 이름을 묻고, 생성 preview 와 명시적 승인을 받기 전에는 `axhub apps bootstrap --execute`, repo 생성, GitHub 연결, 의존성 설치, dev server 시작, 배포를 실행하지 않아요. route label, slash command, skill name, workflow/워크플로, raw question JSON, raw helper JSON, command mapping, raw id, raw email, 파일 경로, 영어 tool title fragment 를 사용자에게 쓰지 않아요.")
