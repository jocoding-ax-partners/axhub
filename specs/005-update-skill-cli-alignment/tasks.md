...
- [x] T014 [US2] ✅ **DONE (사용자 요청으로 본 PR 포함)** — 조사 결과 "값 rename"이 아니라 구조 버그였어요: `catalog::classify()` 가 coarse `error.code` 만 읽어 fine subcode 엔트리가 실제 CLI 봉투(`error.subcode`)로 매치 안 됐어요. fix: (a) `classify()` 를 `error.subcode` 우선 + `error.code` fallback (cli_envelope 선례) 로 변경, (b) `catalog.json` `66:update.cosign_verification_failed` → `66:update.cosign_enforce_failed` (실제 CLI subcode), (c) `bun run codegen:catalog` regen, (d) `phase_parity.rs` + catalog 인라인 테스트를 현실 봉투(code+subcode)로 갱신. 검증: cargo test -p axhub-helpers / bun test / lint:tone 전부 green.
- [x] T015 [US2] ✅ **DONE** — hand-authored `skills/deploy/references/error-empathy-catalog.md` 헤딩 `update.cosign_enforce_failed` 로 정정. **남은 follow-up**: 나머지 catalog subclassified 키(`validation.*`/`env.*`/`github.*`/`open.*`/`profile.*`/`apis.*`/`catalog.*` 15개)는 실제 CLI subcode emit 여부를 신뢰성 있게 대조 못 해서(ax-hub-cli command 전수 audit + 일부는 CLI 가 아직 subcode 를 안 emit 할 수 있음) 미변경. subcode-first + code-fallback 라 회귀 없음(키가 실제 subcode 와 일치하면 자동 동작). 전수 audit 는 별도 작업.
