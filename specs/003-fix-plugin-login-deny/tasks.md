...
- [X] T004a [US1] `state_root()` HOME resolver 정합화 in `crates/axhub-helpers/src/consent/key.rs`: 빈 `XDG_STATE_HOME` 무시, `HOME` → `USERPROFILE` → `HOMEDRIVE`+`HOMEPATH` 순 fallback, 상대경로 `"."` degrade 금지. 빈 XDG 회귀 테스트 추가.
...
- [X] T008 [US1] FR-007 stale 스윕: `sweep_expired_consent_files(dir, key)` 헬퍼(`decode_token_file` 재사용 + `is_consent_token_path`: `consent-*.json` 매칭, best-effort 실패 무시) 추가 in `crates/axhub-helpers/src/consent/jwt.rs`, mint 경로(`mint_token_to_path` 의 `create_dir_all`/`set_private_dir_mode` 직후)와 preauth claim 경로(`claim_pending_token` 의 `read_dir` 직전)에서 호출. 제거 대상은 만료·corrupt·missing-jwt·signature-invalid consent 파일로 한정하고 유효 토큰/unreadable 파일은 보존해요. (T004 후)
- [X] T009 [P] [US1] 스윕 테스트: 만료 pending 파일 + 만료 session consent 파일 + corrupt consent 파일 존재 + mint/preauth 1회 → 해당 stale 파일 제거 / 유효(미만료) pending·session consent 미영향 검증 추가 in `crates/axhub-helpers/tests/cli_e2e.rs` 또는 unit test (T008 후)
...
- [X] T011 [US2] `cmd_preauth_check` deny 출력에 `permissionDecisionReason` 추가(`systemMessage` 제거 금지) + `token_expired` reason 을 TTL 전용 한국어 안내로 매핑 in `crates/axhub-helpers/src/main.rs` (contracts/preauth-check-output.md §2-After)
- [X] T012 [P] [US2] 계약 테스트: deny payload 에 `permissionDecisionReason` + `systemMessage` 둘 다 존재 + `permissionDecision:"deny"` + exit 0 검증, 그리고 session token / pending login-card token TTL 만료 deny 가 모두 `사전 승인이 만료됐어요` 를 표면화하는지 검증 추가 in `crates/axhub-helpers/tests/cli_e2e.rs` (contract C4)
...
- Windows `HOME` 미설정 경계는 본 범위에서 `USERPROFILE` / `HOMEDRIVE`+`HOMEPATH` fallback 으로 정합화해요(research R4).
