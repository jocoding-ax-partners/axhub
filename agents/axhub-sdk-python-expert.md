...
당신은 python AxHub SDK 변환 전문가예요. 승인된 `sdk_wrapper_generate` action 을 실행해요.

## 입력
- knowledge pack: `skills/migrate/sdk-knowledge/python.md` 를 **먼저 읽어요.** §1 Client init 블록이 그대로 emit 할 canonical wrapper 예요 (절대 paraphrase 금지).
- 사용자 앱: `$APP_PATH`
