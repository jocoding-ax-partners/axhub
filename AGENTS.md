...
이 repo 는 axhub Claude Code plugin 이에요. 45 skill 체제에서 **4 skill** (`onboarding` / `init` / `deploy` / `clarity`) 로 다이어트했어요. plugin 은 판정·실행 로직을 직접 갖지 않고 ax-hub-cli (`axhub` 바이너리) 를 호출해요. `clarity` skill 은 나머지 3 skill 에 명확히 안 맞거나 의도가 불분명한 axhub 발화를 라이브 `--help` 트리 탐색으로 명령을 찾아 무확인 실행하는 브리지예요 (카탈로그 내장 금지).
