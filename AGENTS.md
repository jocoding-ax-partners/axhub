...
- NEVER 명시적 결정 없이 skill 추가 — 7 skill 체제 유지 (`clarity` 는 deep-interview spec, `development` 은 office-hours→리뷰 체인, `update` 는 수동 on-demand 업데이트 요청, `import` 는 기존 앱 가져오기 deep-interview→ralplan 승인으로 신설됐어요).
