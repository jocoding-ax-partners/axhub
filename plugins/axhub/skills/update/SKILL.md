...
**섞인 요청 처리.** 업데이트 단계 안에서는 앱 목록·앱 상태·배포 상태·로그·환경변수·데이터 조회를 직접 실행하지 않고, axhub App/MCP 도구도 read 라도 호출하지 않아요. 남은 요청은 결과 카드 뒤에 위 continuation reference 계약대로 이어가요.
