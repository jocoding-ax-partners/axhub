...
# worker 가 남긴 marker(~/.axhub/cache/.plugin-update-restart-codex, 7일 TTL)를 보고, 이
...
M="$HOME/.axhub/cache/.plugin-update-restart-codex"
[ -f "$M" ] || exit 0
...
if ver_ge "$LOADED" "$EXPECTED"; then
  ...
  printf '{"continue":true,"suppressOutput":true,"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"[axhub] 플러그인 새 버전(v%s)을 받았어요. 다음 응답 앞머리에 Codex 를 재시작하면 적용된다고 한 줄만 알리고 확인 명령은 실행하지 마세요."}}\n' "$EXPECTED"
fi
exit 0
