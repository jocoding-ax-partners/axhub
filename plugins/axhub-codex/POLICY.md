...
- `~/.axhub/cache/.plugin-update-check-codex-codex` — 업데이트를 너무 자주 확인하지 않도록 마지막 확인 시각을 남겨두는 표시 파일이에요.
- `~/.axhub/cache/.plugin-update-restart-codex-codex` — 플러그인 새 버전을 받은 뒤 재시작으로 적용됐는지 확인하기 위한 표시 파일이에요.
- 이 파일들은 시각·버전을 표시하는 용도의 작은 파일이고, 개인 정보나 코드 내용은 담지 않아요. Codex 판과 파일을 나눠 써서 두 호스트를 함께 쓰는 컴퓨터에서도 서로의 확인 주기를 방해하지 않아요.
...
- 워크스페이스 AI 활용 기록(프롬프트·응답 수집 기능)의 온보딩 옵트인은 Codex 판 전용 기능이에요. Codex 판 플러그인은 이 수집 설정을 만들거나 바꾸지 않아요.
