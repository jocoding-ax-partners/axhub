...
2. **preview-confirm 명시 텍스트 승인 (필수)**: handoff 전에 만들 table_name·컬럼(name/type, 화이트리스트 text/int/bigint/float/bool/timestamptz/uuid/jsonb)·영향(신규만·기존 데이터 무관)을 한국어로 보여주고 명시 승인받아요.
...
4. **headless 무변경**: CI·비대화형·명시 텍스트 승인 불가면 스키마 변경을 하지 않아요. 기존 테이블이 없으면 필요한 스키마만 보고하고 멈춰요.
