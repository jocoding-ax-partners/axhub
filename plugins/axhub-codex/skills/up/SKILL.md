...
5. 미리보기는 `--dry-run`, 실행은 `--execute` 예요. 승인 전 명령에 `--execute` 를 쓰지 않아요.
6. 성공 선언 — 배포가 끝났다는 말은 `axhub deploy verify` 가 최종 성공을 돌려줄 때만 해요. 시작 명령의 출력·중간 상태·최근 배포 재탐색은 근거가 아니에요.
