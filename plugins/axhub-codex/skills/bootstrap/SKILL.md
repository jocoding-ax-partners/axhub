...
fresh tenant backend는 추천값이에요. 사용자가 발화에서 backend를 명시했다면 그 값을 써요. 아니면 native Question/명시 텍스트 승인 card로 정확히 `이 앱의 코드 저장 위치를 선택해 주세요.`라고 묻고 `GitHub`와 `Axhub self-hosted`를 보여줘요. 추천 반대 옵션도 허용하고, card가 안 보일 때만 같은 선택을 chat text로 물어요.
