...
발화나 같은 대화에 backend가 있으면 쓰고, 없으면 native Question/명시 텍스트 승인 card로 `이 앱의 코드 저장 위치를 선택해 주세요.`라고 묻고 `GitHub`와 `Axhub self-hosted`를 보여줘요. 추천과 다른 선택도 허용해요. provider 선택이 명시되지 않은 headless는 mutation 전에 두 선택지와 재개 문장만 보여주고 멈춰요.
