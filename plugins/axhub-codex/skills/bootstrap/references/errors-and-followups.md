...
Use saga response only for verification. User-facing result should be short Korean lines. URL and public state must be read from the app record through `axhub apps get` CLI only and never synthesized from dry-run subdomain. Do not use `App get (axhub)`, `App list`, deployment MCP, or app connector tools even if Codex shows them as available:
