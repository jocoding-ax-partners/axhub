---
name: diagnosis
description: '배포 실패 원인을 읽기 전용으로 확인할게요. Use for "배포 실패 원인 진단해줘", "최근 배포 실패 원인만 읽기 전용으로 진단해줘", "왜 배포가 죽었어", "재배포는 하지 말고 원인만 봐줘", "diagnose failed deployment <id> for app <slug>", "why did my deploy fail". Start directly with that Korean sentence; no preamble; do not explain the trigger, routing, or session-start instructions. 스킬 실행 전 사용자 문장·App/MCP·파일 조회는 0개예요. MCP로 답하거나 재배포·롤백하지 않고 다른 플랫폼 실패는 양보해요.'
...
- 사용자에게 보이는 진행 문구와 Bash/tool call 제목은 한국어로만 써요. shell/run tool도 같은 규칙을 따르고, 실제 호출 제목은 `실패 배포 상태 확인`, `실패 배포 로그 확인`, `현재 라이브 상태 확인` 처럼 짧은 한국어 명사구로 고정해요.
- 각 CLI 확인은 별도 tool 로 실행해요. status+logs, 설치 확인+help 같은 여러 명령을 한 카드에 묶지 않아요. 중간 요약도 `실패 배포 확인 완료`, `현재 라이브 상태 확인` 처럼 한국어로만 남겨요.
...
- `axhubing`, `axhubed`, `diagnosing`, `checking` 처럼 제품명이나 영어 동사를 붙인 도구 제목을 쓰지 않아요. 예를 들어 `axhubed CLI 확인` 대신 `현재 라이브 상태 확인` 을 써요. 도구 제목에는 제품명 `axhub` 자체도 넣지 않아요.
- Codex 에 보이는 명령은 항상 `axhub ...` 로 시작하는 **한 개의 bare 명령**이에요. `CLI_NAME`, `CLI_BIN`, 변수 대입, `command -v`, `if`, command substitution, `bash -lc`, `sh -c`, `;`, `&&`, `||`, pipe, redirect, `echo`, `grep`, `head`, `tail`, `2>/dev/null`, `$?`를 절대 쓰지 않아요. 명령 실패와 exit code 는 tool 결과에서 직접 읽어요.
...
`codex exec`·CI·TTY 없음이면 헤드리스예요. 헤드리스에선 앱 이름을 되묻지 않고 단서로만 좁히고, 못 좁히면 `대상을 못 찾았어요` 로 끝내요. CLI 호출 자체는 헤드리스에서도 동작해요.

## Workflow

1. **대상 추론.** 다음 우선순위로 앱을 좁혀요.
   - `deploy` handoff가 준 검증된 app id/slug와 deployment id
   - 같은 대화에서 성공 응답으로 검증된 app id/slug
   - 현재 폴더의 `.axhub/import-resume.json` 또는 bootstrap resume에 저장된 `app_id`
   - origin 저장소 이름 또는 `axhub.yaml`의 `name` 후보

   `axhub.yaml`의 `name`은 로컬 프로젝트 이름일 수 있으므로 원격 앱 slug로 단정하지 않아요. app id가 있으면 bare `axhub apps get <app-id> --json` 한 번으로 검증해요. manifest/origin 후보만 있으면 bare `axhub apps get <candidate> --json`을 한 번만 호출하고, not-found면 `apps list`, `--help`, pipe 탐색으로 복구하지 말고 대화형에서 앱을 한 번 물어요. 헤드리스에선 `대상을 못 찾았어요`로 끝내요.

2. **표면 실행 — CLI 전용·탐색 없음.**
   - 이 세션에 `deployment_diagnosis` 같은 MCP 도구가 callable 이어도 사용하지 않아요.
   - 공개 표면은 이미 고정돼 있어 `command -v`나 세 개의 `--help`를 먼저 돌리지 않아요. 아래 필요한 bare 명령만 실행해요. command-not-found/unknown-command면 `진단을 못 했어요`로 끝내고, 다른 철자나 help tree를 탐색하지 않아요.
     ...
     axhub deploy logs --app <앱> --since <시작> --until <종료> --json --limit 100
     ```

     `deploy logs` 는 앱 단위 로그예요 — 시간창으로 좁혀도 요약은 "그 배포 시간대의 앱 로그"로 정직하게 말해요.

     각 tool 제목은 `실패 배포 상태 확인`, `실패 배포 로그 확인` 으로 쓰고 반드시 별도 카드로 실행해요.
...
3. **결과 분류.** 실패 배포가 있으면 deployment status/logs 를 먼저 원인군으로 접고, 앱 진단은 현재 라이브 상태로 따로 접어요. 앱 진단 필드는 `applicable`, `healthy`, `services[].healthy`, `services[].reason{stage,code,category,message}`, `services[].build_log_tail`(있으면), `signals[]` 를 봐요. 실패 배포 status 응답에서는 최상위 `build_log_tail` 도 봐요.
   ...
   **docs 대조 — 원인이 안 좁혀지면 헤매지 말고 문제 해결 문서를 읽어요.** status 의 실패 사유와 로그의 마지막 에러 줄을 봤는데도 원인군을 한 문장으로 못 접겠으면(예: 파일이 있는데 `spawn … ENOENT`, 알 수 없는 exit, 낯선 런타임 에러), CLI 를 더 돌리지 말고 `https://docs.axhub.ai/llms-full.ko.txt` 를 한 번 가져와 `# 문제 해결` 절의 "자주 나는 원인 표"와 "헬스체크 실패 사유 코드" 표에서 같은 증상 행을 찾아요. 행이 맞으면 그 행의 "확인할 것"을 해결 후보로 옮겨 말하고, 없으면 원인군 요약만 해요. 이 조회는 읽기 전용이고 한 번만 해요 — 같은 URL 을 반복 fetch 하거나 다른 페이지를 돌아다니지 않아요. 영어 대화면 `llms-full.en.txt` 를 써요.
