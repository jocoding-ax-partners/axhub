...
- 업데이트 뒤 같은 요청 안에 앱 현황 확인이 남아 있으면 존재하지 않는 `axhub app list` 를 추측하지 않고, plural `axhub apps` 표면을 help 로 확인한 뒤 정확히 `axhub apps list --json` 같은 읽기 전용 명령으로 이어가요. 관련 앱을 고른 뒤에도 `axhub apps get <app> --json`, `axhub deploy list --app <app> --json` 처럼 실제 CLI 표면만 쓰고, 존재하지 않는 `axhub deployment list` 나 `| head`, `2>/dev/null`, `grep` 같은 shell 후처리는 붙이지 않아요.
