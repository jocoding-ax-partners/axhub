...
- `bun run plugin:budget` — 컨텍스트 byte 예산 PASS (SKILL.md 11개 합산 245k · per-skill 35k). codex 번들은 `bun run plugin:budget:codex` 로 같은 예산을 별도 검사해요. 넘치면 본문을 reference 로 추출하되, **실행 경로에 필요한 지시는 빼지 않아요** — reference 는 plugin cache 라 workspace 밖이고 Desktop 은 읽을 때 권한 프롬프트를 띄우는데 우리 규칙(bootstrap 9.1)은 그 프롬프트를 생략하라고 해서 조용히 안 읽혀요. 참고용 상세만 reference 로 가요.
