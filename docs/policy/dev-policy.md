...
- diet 체제는 스킬 수를 최소로 유지하는 방침이에요. 공개 skill은 10개(onboarding/bootstrap/scaffold/plugins/import/deploy/development/diagnosis/clarity/update)를 유지해요.
- 새 skill은 기존 10개의 경계·양보 규칙으로 연결할 수 없는 사용자 의도가 반복 관측될 때만 추가해요.
...
- 10개 SKILL.md의 frontmatter는 `tests/frontmatter.test.ts`로 검증해요. `name`·`description`은 host가 어떤 스킬을 쓸지 고르는 기준이라 깨지면 스킬이 선택되지 않을 수 있어요.
...
- `bun run plugin:budget` — 컨텍스트 byte 예산 PASS (SKILL.md 10개 합산 220k · per-skill 35k). codex 번들은 `bun run plugin:budget:codex`로 같은 예산을 별도 검사해요. 넘치면 실행 경로가 아닌 참고 상세만 reference로 옮겨요.
