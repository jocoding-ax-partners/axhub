...
- 8 agent md: 3 quality (`axhub-reviewer`, `axhub-debugger`, `axhub-shipper`) + 5 migrate planning (`axhub-migrate-discoverer`, `axhub-migrate-planner`, `axhub-migrate-architect`, `axhub-migrate-critic`, `axhub-migrate-reviewer`) — migrate full_consensus 단계에서 직접 dispatch 돼요
