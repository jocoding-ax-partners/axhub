...
| 7 | **TLS proxy 정책** | 사내 corporate proxy / TLS 검사 (MITM) 가 있는 환경에서 OAuth / API 호출이 통과하는지 | CLI/OS 신뢰 store 에 회사 root CA 를 추가하고 `HTTPS_PROXY`/endpoint 정책을 표준화 |
...
📌 5 분 안에 첫 배포까지 따라 할 수 있는 가이드:
   👉 https://github.com/jocoding-ax-partners/axhub/blob/main/docs/vibe-coder-quickstart.ko.md

📌 막혔을 때 보는 troubleshooting:
   👉 https://github.com/jocoding-ax-partners/axhub/blob/main/docs/troubleshooting.ko.md
...
| `cosign_enforce_failed` | `axhub update apply` 의 exit 66 카운트 | 0 건. 발생 시 [사고 대응 runbook](#사고-대응-runbook) 즉시 |
...
| `AXHUB_TOKEN=<token>` | 일회성 headless / CI 인증 폴백. | 미설정 (keychain 우선) | 장기 보관 금지. 지속 저장은 helper `token-import` → `~/.config/axhub-plugin/token` (0600) 사용 |
...
### Severity 3 — `cosign_enforce_failed`
...
- `cosign_enforce_failed`, `update_apply_succeeded`
...
- `cosign_enforce_failed` 발생: 즉시 관리자 contact, 절대 강제 우회 금지.
