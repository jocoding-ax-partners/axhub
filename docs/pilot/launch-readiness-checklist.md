...
## 별첨: current release verification baseline (v0.1.19)

**Pilot 권장 release: v0.1.19** (current plugin metadata version). The v0.1.9 notes below are retained as historical Windows BOM baseline context.

Pilot 시작 전 plugin v0.1.19 release cosign signature + sha256 무결성 검증 결과:

- 검증 명령: `bash scripts/release/verify-release.sh v0.1.19`
- 검증 시점: 2026-04-27
- 검증자: jocoding-ax-partners (Phase 11 ralph)
- 검증 결과: **✓ All release assets verified for jocoding-ax-partners/axhub@v0.1.19**
- Live binary self-report (darwin-arm64): `axhub-helpers 0.1.19 (plugin v0.1.19, schema v0)` ✓
...
**Historical release: v0.1.7** (Phase 10 — Windows PS1 hooks 추가. Git Bash/WSL 없이도 stock Windows 10/11 에서 plugin end-to-end 작동. Claude Code >= 2.1.84 필수).
...
2. macOS 에서 wrong-OS `"shell":"powershell"` spawn 동작 silent 가정은 **거짓으로 확인됨**. Claude Code는 PowerShell 미설치 호스트에서 non-blocking hook error를 노출한다. Current fix removes the unconditional PowerShell SessionStart sibling from universal `hooks/hooks.json`; stock Windows automatic SessionStart needs future platform-specific hook packaging.
...
- **v0.1.6**: Windows keychain 통합 작동했지만 install.sh + session-start.sh 가 bash-only — Git Bash/WSL 없는 stock Windows 사용자는 plugin 자동 설치/SessionStart 모두 broken. v0.1.7 added bin/install.ps1 + hooks/session-start.ps1 + hooks.json sibling powershell entry, but later evidence showed the universal sibling creates visible startup errors on non-Windows hosts. Current universal hook config keeps only the bash SessionStart shim until platform-specific hook packaging exists.

신규 install + pilot 은 **v0.1.19** 사용.
