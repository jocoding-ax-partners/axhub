...
**상태**: v0.9.28 · 42 SKILL / 9 command / 3 quality sub-agent / 5 cross-arch cosign-signed binary 라이브.
...
4. **[§4 동작 원리](#4-동작-원리)** — 라우팅·SKILL·consent·auth 같은 메커니즘 깊이 파기
...
│    preflight · deploy-prep · resolve · consent-mint · preauth-check   │
...
| destructive 명령 consent 게이트 (HMAC) | backend 직접 호출 (→ ax-hub-cli) |
...
│   ├── post-tool-verify-deploy-artifacts.ts  # 배포 artifact 검증 (Bun)
│   └── _helpers.ts           # TS hook 용 kill-switch 헬퍼 (Rust 미러)
├── crates/axhub-helpers/     # Rust helper 바이너리 (이 플러그인의 코어)
│   ├── src/main.rs           # argv 디스패치 + 모든 서브커맨드 핸들러 (~3000줄)
│   ├── src/*.rs              # 모듈 (preflight/hook_safety/consent/keychain/...)
...
| | `preauth-check` | PreToolUse: destructive `axhub` 명령 consent 게이트 | `main.rs:992` |
...
| | `consent-mint` / `consent-verify` | HMAC-JWT consent 토큰 발급/검증 (§4.4) | `main.rs:806` / `829` |
...
| `consent/*` | HMAC-JWT consent 토큰 (jwt/key/parser/schema/decision) |
...
| HMAC consent 키 | `$XDG_STATE_HOME/axhub/hmac-key` (32바이트, 최초 사용 시 생성) |
| consent 토큰 (세션별) | `$XDG_RUNTIME_DIR/axhub/consent-<session_id>.json` (XDG_RUNTIME_DIR 미설정 시 `$XDG_STATE_HOME/axhub/runtime/`, 기본 `~/.local/state/axhub/runtime/`) |
...
| `PreToolUse` | `Bash` | `axhub-helpers preauth-check` | 5s |
...
| `PostToolUse` | `Bash` | `bun hooks/post-tool-verify-deploy-artifacts.ts` | 7s |
...
truthy 값: `1`/`true`/`yes`/`on`. shell wrapper(`session-start.sh`)·Rust(`hook_safety.rs`)·TS(`_helpers.ts`) 세 곳이 **동일 로직을 미러**해요 — shell 에서 먼저 거르면 binary cold-start 비용도 안 들어요.
...
### 4.4 consent 토큰 — destructive 명령 게이트

> 이름이 비슷한 두 가지를 먼저 구분해요. `consent-mint`/`consent-verify`/`preauth-check` 는 **HMAC-JWT 게이트**(이 절). 반면 그냥 `consent` 서브커맨드는 megaskill on/off 플래그라 완전히 별개예요.

**메커니즘** (`consent/jwt.rs`, `consent/key.rs`):
- `axhub deploy create` 같은 destructive `axhub` 명령이 Bash 로 실행되기 직전, PreToolUse `preauth-check` 가 그 명령을 파싱해요(`parser.rs`). destructive 면 `{action, app_id, profile, branch, commit_sha}` 바인딩으로 **HMAC-JWT(HS256)** 토큰을 검증해요.
- 유효한 consent 토큰이 있으면 `permissionDecision: "allow"`, 없으면 `"deny"` + "consent 카드를 띄우려면 이렇게 말해요" 한국어 힌트.
- 키는 `$XDG_STATE_HOME/axhub/hmac-key`(32바이트, 최초 생성), 토큰은 `$XDG_RUNTIME_DIR/axhub/consent-<session>.json`(XDG_RUNTIME_DIR 미설정 시 `${XDG_STATE_HOME:-$HOME/.local/state}/axhub/runtime/` — Claude Code 처럼 프로세스마다 `$TMPDIR` 가 다른 환경에서도 mint/read 가 같은 경로를 보게 함), 기본 TTL 60초.
- 파일 I/O 는 `O_NOFOLLOW` + symlink 거부 + 0o600 권한으로 심링크/권한 공격을 막아요(`key.rs`).
- `CLAUDE_SESSION_ID` 가 없을 때를 위한 **pending 토큰** 흐름도 있어요 — 세션 전에 민트한 consent 를 세션이 생기면 claim 해서 굴려요.

읽기 전용 명령은 항상 통과해요. 이 게이트가 "preview 텍스트"가 아니라 **실제 차단 지점**이에요(ADR-0009).
...
[Step 3] preview card (AskUserQuestion) — 앱/환경/브랜치/커밋/예상시간 5필드 → [네 배포 / 미리보기만 / 취소]
   │      "취소" = consent 안 만듦 · "미리보기만" = --dry-run
   │
[Step 3.5] token-gate — 토큰 신선도 폴링(최대 30s); exit 65 → Step 6 복구
   │
[Step 4] consent-mint + deploy create
   │      echo '{"action":"deploy_create",...}' | axhub-helpers consent-mint   (HMAC 토큰 발급)
   │      axhub deploy create --app ... --json     (← PreToolUse preauth-check 가 consent 검증 후 통과)
...
| 배포 중 토큰 만료 | Step 3.5 token-gate 가 **mutation 전에** 잡아 `auth` 로 — consent 민트 전에 차단 |

핵심은 **fail-safe 순서**예요: 위험한 작업(consent mint, deploy create) 앞에 항상 게이트(preflight, token-gate, preauth-check)가 있어서, 인증·상태가 깨진 채로 mutation 이 일어나지 않아요.
...
# 5. consent/세션 상태 직접 보기
ls "$([ -n "$XDG_RUNTIME_DIR" ] && echo "$XDG_RUNTIME_DIR/axhub" || echo "${XDG_STATE_HOME:-$HOME/.local/state}/axhub/runtime")/"   # consent-*.json
...
| ADR | 결정 | 핵심 이유 |
|-----|------|-----------|
| [0009](docs/adr/0009-free-form-preview-policy.md) | free-form preview 카드 허용 | 강제 지점은 prose 가 아니라 Rust consent 게이트. 풍부한 배포 식별 정보는 작은 option 스키마에 안 맞음 |
...
### 안전 가드 (요약)
- **HMAC consent 토큰** — destructive op 차단 (`CLAUDE_SESSION_ID` 바인딩, O_NOFOLLOW, symlink reject) — §4.4
...
`ms`=multi-step, `pf`=needs-preflight.

| SKILL | 트리거 예시 | 슬래시 | ms | pf | model |
|-------|-------------|--------|----|----|-------|
| `deploy` | "내 앱 배포해", "ship" | `/axhub:deploy` `/axhub:배포` | ✅ | ✅ | sonnet |
| `migrate` | "기존 앱 올려줘", "migrate this repo" | — | ✅ | ✅ | sonnet |
| `status` | "방금 배포 어떻게 됐어" | `/axhub:status` | — | — | haiku |
| `logs` | "빌드 로그 보여줘" | `/axhub:logs` | — | — | haiku |
| `recover` | "방금 거 되돌려" | — | ✅ | ✅ | sonnet |
| `apps` | "내 앱 목록" | `/axhub:apps` | — | ✅ | sonnet |
| `auth` | "axhub 로그인해줘" | `/axhub:login` | — | — | sonnet |
| `update` | "axhub CLI 새 버전 있어" | `/axhub:update` | ✅ | — | sonnet |
| `upgrade` | "플러그인 업그레이드" | — | ✅ | — | sonnet |
| `doctor` | "axhub 설치돼 있어" | `/axhub:doctor` | ✅ | — | haiku |
| `init` | "결제 앱 만들어줘" | — | ✅ | — | sonnet |
| `setup` | "처음인데 셋업해줘" | — | ✅ | — | sonnet |
| `install-cli` | "axhub CLI 설치해줘" | — | ✅ | — | sonnet |
| `env` | "환경변수 뭐 있어" | — | ✅ | ✅ | sonnet |
| `github` | "GitHub repo 연결해" | — | ✅ | ✅ | sonnet |
| `open` | "결과 봐" | — | — | — | haiku |
| `profile` | "회사 endpoint 바꿔" | — | ✅ | — | sonnet |
| `my-resources` | "내가 쓸 수 있는 리소스" | — | — | ✅ | sonnet |
| `data` | "데이터 카탈로그 검색" | — | ✅ | ✅ | sonnet |
| `verify` | "진짜 배포됐는지 확인" | — | ✅ | ✅ | — |
| `trace` | "왜 실패했는지 추적" | — | ✅ | ✅ | sonnet |
| `clarify` | (모호 발화 fallback) | — | — | — | haiku |
| `routing-stats` | "라우팅 통계" | — | — | ✅ | haiku |
| `enable-statusline` | "statusline 켜줘" | — | — | — | haiku |
| `axhub-review` | "코드 리뷰해줘" | — | ✅ | ✅ | sonnet |
| `axhub-debug` | "이거 디버그해줘" | — | ✅ | ✅ | sonnet |
| `axhub-diagnose` | (배포/테스트 실패 자동 진단) | — | ✅ | ✅ | sonnet |
| `axhub-ship` | "리뷰 통과했으니 배포" | — | ✅ | ✅ | sonnet |
| `axhub-tdd` | "TDD 로 짜줘" | — | ✅ | ✅ | sonnet |
| `axhub-plan` | "개발 계획 세워줘" | — | ✅ | ✅ | sonnet |
| `using-axhub-quality` | (품질 모드 안내) | — | — | — | sonnet |
| `karpathy-guidelines` | (코딩 가이드 참조) | — | — | — | sonnet |
| `app-lifecycle` | "앱 복제", "앱 일시정지", "앱 재개" | — | ✅ | ✅ | sonnet |
| `rollback` | "이전 배포로 롤백" | — | ✅ | ✅ | sonnet |
| `tables` | "테이블 만들", "컬럼 추가", "행 넣어" | — | ✅ | ✅ | sonnet |
| `connectors` | "DB 연결", "커넥터 추가", "postgres 연결" | — | ✅ | ✅ | sonnet |
| `resources` | "리소스 이름 바꿔", "리소스 정리", "네임스페이스 만들" | — | ✅ | ✅ | sonnet |
| `publish` | "앱 공개", "마켓에 올려", "심사 제출" | — | ✅ | ✅ | sonnet |
| `browse` | "앱 둘러봐", "템플릿 뭐 있어", "공개 앱 찾아" | — | ✅ | ✅ | haiku |
| `team` | "팀원 초대", "초대 목록", "접근 권한" | — | ✅ | ✅ | sonnet |
| `workspace` | "내 워크스페이스", "테넌트 목록" | — | ✅ | ✅ | haiku |
| `inspect` | "매니페스트 확인", "axhub.yaml 검증", "설정 확인" | — | ✅ | — | haiku |
...
| `CLAUDE_SESSION_ID` | consent 토큰 바인딩 키 |
...
| **consent token** | destructive 명령 직전 검증하는 HMAC-JWT (≠ megaskill `consent` 플래그) |
...
| 분류 | 서브커맨드 |
|------|-----------|
| **hook 진입점** | `session-start` · `prompt-route` · `preauth-check` · `classify-exit` |
| **consent (HMAC-JWT)** | `consent-mint [--validate-only]` · `consent-verify` |
...
> **정직한 디테일**: `USAGE` 문자열은 일부 hook 서브커맨드를 안 적어놨어요. `hooks.json` 이 실제로 호출하지만 `USAGE` 에 없는 것들 — `commit-gate` · `tdd-inject` · `test-classifier` · `state-update` · `karpathy-inject`, 그리고 megaskill on/off 플래그인 `consent`(§4.4 의 consent-mint 와 **다른 것**) — 도 `run()` 의 `match` 에는 존재해요. `main.rs` 를 읽을 땐 `USAGE` 가 아니라 `match` arm 이 authoritative 예요.
...
| `consent/{jwt,key,parser,schema,decision}` | HMAC-JWT 발급/검증, 명령 파싱, 바인딩 스키마, 4-variant 결정 |
...
| `runtime_paths` / `config` / `redact` / `humanize` / `spawn` / `session_bundle` / `hook_output` | XDG 경로, 유저 config, 비밀 redaction, 한국어 시간 표기, 서브프로세스, 세션 번들, hook envelope |
...
| `jsonwebtoken` + `hmac` + `sha2` + `base64` | consent HMAC-JWT(HS256), audit 해시 |
| `getrandom` | consent HMAC 키 32바이트 난수 생성 |
...
| SKILL 워크플로우 | 본문 bash 의 in-body preflight + `deploy-prep`/`consent-mint`/`token-gate` 등 (helper-pick fallback) |
