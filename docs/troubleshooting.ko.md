...
keychain 이 사용 불가한 환경 (특정 Linux 컨테이너 등) 에서는 helper 의 `token-import` 흐름이 토큰을 `~/.config/axhub-plugin/token` 에 `0600` 권한으로 저장합니다. 자동화 환경에서는 임시 파일 경로를 공유하지 말고, 필요한 명령에 한해 `AXHUB_TOKEN` 환경변수나 위 plugin token 경로를 사용하세요.
...
3. 또는 본인 PC 에서 토큰을 받은 뒤 (`axhub auth token export`) 그 값을 helper 로 import:
   ...
   # Codespaces 에서
   cat /tmp/axhub-token.json | axhub-helpers token-import
   AXHUB_TOKEN="$(cat "$HOME/.config/axhub-plugin/token")" axhub auth status --json
   ```

import 된 plugin token 파일은 `~/.config/axhub-plugin/token` 에 `0600` 권한으로 저장됩니다. 임시 export 파일(`/tmp/axhub-token.json`)은 import 직후 삭제하세요.
