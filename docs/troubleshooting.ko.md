...
keychain 이 사용 불가한 환경 (특정 Linux 컨테이너, Windows headless 등) 에서는 helper 의 `token-import` 흐름이 토큰을 `~/.config/axhub-plugin/token` 에 안전하게 저장해요. POSIX 계열은 `0600` 권한을 확인하고, Windows native 는 명시적 PowerShell/helper 경로를 사용해요. 자동화 환경에서는 임시 파일 경로를 공유하지 말고, 필요한 명령에 한해 `AXHUB_TOKEN` 환경변수나 위 plugin token 경로를 사용하세요.

---

### 실수로 잘못 배포했어요. 되돌릴 수 있나요?

**현재 axhub CLI 는 자동 rollback 명령을 제공하지 않아요.** 대신 권장하는 방법은 **forward-fix** (앞으로 고쳐서 새 배포로 덮어쓰기) 예요.

1. 로컬 코드를 이전 버전 / 정상 버전으로 되돌리기 (git revert, 또는 마지막으로 잘 돌던 commit 으로 checkout).
2. commit & push.
3. `paydrop 다시 배포해` — 새 배포가 prod 를 정상 버전으로 덮어써요.
4. 약 3-5 분 후 정상 복구.
...
```bash
# 설치 위치 찾기
which axhub          # macOS / Linux / Git Bash / WSL
where.exe axhub      # Windows cmd

# 결과가 없으면 PATH 환경변수 확인
echo $PATH           # macOS / Linux / Git Bash / WSL
```
...
Homebrew 경로 (보통 `/opt/homebrew/bin` 또는 `/usr/local/bin`) 가 PATH 에 있는지 확인. Windows native 는 Scoop 경로나 회사 배포 경로가 PATH 에 있는지 확인해요. 없으면 `~/.zshrc` (macOS) 또는 `~/.bashrc` (Linux/Git Bash/WSL) 에 추가하고, Windows 는 사용자 PATH 를 갱신한 뒤 새 터미널을 여세요.
...
2. 플러그인이 headless 환경을 감지하고 다음과 같이 응답:
   ...
   1. 아래 URL 을 본인의 PC 또는 폰의 브라우저에 복사해서 여세요:
      https://axhub-api.jocodingax.ai/device
...
3. 또는 본인 PC 에서 토큰을 받은 뒤 (`axhub auth token export`) 그 값을 helper 로 import:

   ```bash
   # POSIX: macOS / Linux / Git Bash / WSL
   axhub auth token export > "$HOME/axhub-token.json"

   # Codespaces / SSH / CI 등 headless 환경에서
   ${CLAUDE_PLUGIN_ROOT}/bin/axhub-helpers token-import < "$HOME/axhub-token.json"
   AXHUB_TOKEN="$(cat "$HOME/.config/axhub-plugin/token")" axhub auth status --json
   rm -f "$HOME/axhub-token.json"
   ```

   ```powershell
   # Windows PowerShell
   axhub auth token export | Set-Content -Path "$env:USERPROFILE\axhub-token.json" -Encoding utf8
   Get-Content "$env:USERPROFILE\axhub-token.json" | & "$env:CLAUDE_PLUGIN_ROOT\bin\axhub-helpers.exe" token-import
   $env:AXHUB_TOKEN = (Get-Content "$env:USERPROFILE\.config\axhub-plugin\token" -Raw).Trim()
   axhub auth status --json
   Remove-Item "$env:USERPROFILE\axhub-token.json" -Force
   ```

import 된 plugin token 파일은 `~/.config/axhub-plugin/token` 에 저장돼요. POSIX 계열은 helper 가 `0600` 권한을 확인해요. 임시 export 파일은 import 직후 삭제하세요.
...
| **플러그인 자체 버그 / 기능 요청** | https://github.com/jocoding-ax-partners/axhub/issues |
