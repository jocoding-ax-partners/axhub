...
- KTD12. **최소 버전 게이트 + compat matrix** — 지원 버전은 Codex CLI ≥ 0.147.0(전 실측의 검증 기준선)이에요. dev-policy 에 의존 내부 거동 6개(trust hash 대상, 8,000B 절단, 1,024자 절단, 카탈로그 2% 예산, manifest 우선순위, `CLAUDE_PLUGIN_ROOT` 호환 env — `PLUGIN_ROOT` 네이티브 치환 여부는 U1-(p) 실측으로 확정해 의존 제거)를 compat matrix 로 명시하고 분기별 재검증 항목으로 둬요. matrix 는 last-verified codex 버전을 기록하고 U10 QA 기록이 그 값을 갱신하며, U10 QA 는 최소 버전(0.147.0)과 검증 시점 최신 버전 두 지점에서 수행해 tested range 를 남겨요. 지원 문구는 "≥ 0.147.0 (최종 검증: <버전>)" 형식이에요. **2026-08-19 U1 실측(0.148.0):** bypass 플래그가 `--dangerously-bypass-hook-trust` 로 개명(0.147 표기 폐기), `PLUGIN_ROOT`·`CLAUDE_PLUGIN_ROOT` 둘 다 네이티브 주입(치환 불요 — 의존 제거 확정), exec 레인 스킬 본문 절단 부존재((k) — matrix 의 8,000B 항목은 TUI 주입 전제로 재정의), `codex exec` 자동화는 stdin 개방 시 무기한 대기라 `</dev/null` 필수, `plugin list --json` 은 installed[] 만 신뢰(available[] 는 빈 배열), 로컬 marketplace 는 `upgrade` 불가(Git 전용)·재 `plugin add` 가 in-place 갱신 경로예요.
...
- **Dependencies:** U1 실측 — (c)(h)(p)·(k-exec) 확정으로 착수 가능해요. (b)·(k-TUI) 는 U6 코어 물량·U8 byte-offset 활성에만 게이트예요. U2, U3, U4.
...
- **Dependencies:** U5 (스왑 메커니즘). U1 실측 반영 — update lane·README/POLICY codex 판·descriptions.json·SOURCE_HASHES 등 비코어 물량은 즉시 착수 가능하고, 실행 4스킬 8KB 코어(+조건부 scaffold·clarity)는 TUI 주입 실측((b)·(k-TUI)) 판정까지 보류 lane 이에요 (KTD4).
...
- **Approach:**
  1. `.agents/plugins/marketplace.json` — axhub(`./plugins/axhub`)·axhub-codex(`./plugins/axhub-codex`) 두 엔트리, version 생략(KTD3). Claude 번들 엔트리의 codex installation 차단 policy 는 U1-(d) 실측으로 기각됐어요 — codex 는 매니페스트의 installation/policy 필드를 무시해요(파싱 허용·미매핑). R12 fallback 은 차단이 아니라 안내로 제공해요(U9 README·스킬 안내). 노출 스테이징: axhub-codex 엔트리는 U8 게이트 green + U10 QA 통과 후에만 main 에 병합해요 — QA 는 병합·태그 전 릴리즈 브랜치에서 local marketplace 등록으로 수행하므로(U10) 미검증 번들이 공개 설치 가능해지는 창이 아예 없어요(R11 확장). (U1-(d) 실측: policy 미지원 확정 — 이중 안전은 문서 안내로 대체해요.)
...
- **Test scenarios:**
  ...
  - `.agents` 각 엔트리 name 이 대상 번들 plugin.json name 과 일치해요 (U1-(l) 실측: 불일치는 설치 거부 hard error).
- **Verification:** 격리 `CODEX_HOME` 에서 repo 등록 후 `codex plugin list` 에 axhub-codex 가 보여요 — U1-(o) 실측대로 available 판정은 표 출력 기준이고 `--json` 은 installed[] 만 신뢰해요. Claude 번들 엔트리는 codex 에서도 설치 가능하므로(차단 policy 미지원) 안내 문구 제공 여부를 U9 산출물에서 확인해요.
...
- **Test scenarios:**
  ...
  - R3 byte-offset assert (4스킬 × 게이트 문자열 — U6 이 scaffold·clarity 코어를 추가한 분기에선 대상 확장). **TUI 실측 판정 전 보류** (U1-(k) exec 실측: 절단 부존재, KTD4 보류 lane).
  - 실행 스킬 codex 코어의 파일 크기 ≤ 8,000B assert (KTD4 선행조건 자체 검사 — 조건부 코어 포함). **TUI 실측 판정 전 보류** (동일 게이트).
...
**2026-08-19 U1 실측(0.148.0)으로 원 4항목은 전부 해소됐어요** (16항목 중 13 확정 — 세부는 세션 작업 노트):

- `.agents` 로컬 상대경로 source: 동작 확정. installation policy: 미지원 확정(필드 무시) → U7 은 차단 대신 안내로 확정. `.agents` 부재 시 codex 는 legacy `.claude-plugin` 을 읽어요(fallback 확정) — 공존 시엔 `.agents` 완전 가림.
- 훅 outer `continue`: 수용 확정 (Completed·additionalContext 주입·모델 인용). invalid JSON stdout 훅은 "Failed" 표기만 되고 세션 비차단.
