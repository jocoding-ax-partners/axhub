# Onboarding 스킬 진화 — CEO Review Plan

> `/plan-ceo-review` 산출물. Mode: **SELECTIVE EXPANSION** (onboarding 현 스코프 = baseline, 추가분 cherry-pick).
> 결정일: 2026-06-05 · Branch: main · Reviewer: Claude (Opus 4.8)

## TL;DR

Node.js 바이브코더가 "설치 → 로그인 → 개발 시작" 을 한 흐름으로 끝내도록 **`onboarding` 스킬을 진화**해요.
`onboarding` 스킬 이름으로 정리해요 — onboarding 이 이미 온보딩 오케스트레이터고
`온보딩`·`getting started`·`onboarding` 트리거 어구를 소유하고 있어서, 병렬 스킬은 keyword baseline 을
깨고 첫사용 스킬 2개가 트리거를 두고 경쟁해요 (DRY 위반). 사용자 의도도 "리팩토링" 이에요.

**제품 계약:** 사용자는 `온보딩`, `처음인데 뭐부터`, `getting started` 한 마디만 하면 돼요. 내부 slug 는
`onboarding` 으로 정리하고, 사용자 경험상 이 스킬이 **온보딩 단일 진입점**이에요. 온보딩 중 사용자가 직접
알아야 하는 sibling skill 이름(`install-cli`/`auth`/`github`/`init`/`deploy`/`doctor`)은 없어야 해요.
onboarding 이 감지→위임→재감지→다음 gap 을 끝까지 닫고, 마지막에는 **VIBE_READY 카드**를 보여줘요.
...
온보딩의 종료 조건은 "설치 끝" 이 아니라 **바로 바이브코딩 가능한 상태**예요. `onboarding` 은 아래 조건 중
...
- `READY_WITH_USER_ACTION` — 브라우저 승인, OS installer GUI, PATH reload 처럼 에이전트가 대신 완료할 수
  없는 외부 행동만 남음. 반드시 "승인했어/다시 온보딩해줘" 같은 자연어 재개 문구를 제공해요.
...
- **Q: 새 스킬이 맞나?** → 아니요. `onboarding` 이 이미 detect-first 오케스트레이터예요. 신규 가치는
  *버전체크 레이어* + *GitHub 연결 전진배치* 두 개뿐. 나머지는 onboarding 이 이미 함.
- **Q: onboarding 이 god-skill 되지 않나?** → 핵심 규율: onboarding 은 **오케스트레이션만** 해요. 모든 로직은
  기존 스킬에 위임 (`install-cli`/`auth`/`update`/`upgrade`/`github`/`init`/`doctor`/`deploy`).
  onboarding 이 직접 소유하는 건 (a) phase 순서, (b) node 런타임, (c) 신규 의존성 설치뿐.
...
온보딩 오케스트레이터 (= onboarding)
 ...
 ├ git 런타임          → onboarding 직접     [신규] consent 설치 (clone/push 전제조건)
 ├ node 런타임         → onboarding 직접     (consent 설치) + [신규] 버전 자동교정
 ├ GitHub 연결 전진배치 → github(재사용)  [신규 순서] App 설치(install_url)만 auth 뒤 1회
 ├ 의존성 설치         → onboarding 직접     [신규] repo 생긴 뒤 pm 감지 + consent install --ignore-scripts
...
### Accepted (이번 PR)
0. **온보딩 단일 진입점 계약** — `onboarding` 을 사용자-facing onboarding skill 로 강화. 사용자는 sibling skill
   이름이나 slash command 를 몰라도 돼요. 모든 gap 은 onboarding 이 detect-first 로 찾아 위임하고, 위임 후
...
3. **git/node 런타임 보장** — detect-first 에 `git --version` 추가. git 미설치면 consent 설치
   ...
   런타임 불일치 시 consent 후 nvm 교정 (현재 onboarding 은 경고만). git 은 node 에 이은 **2번째
...
### NOT in scope
- **새 `onboarding` 스킬 신설** — 트리거 충돌 + baseline 깨짐으로 기각. onboarding 진화로 대체.
...
- **무동의 100% 자동화** — "100%" 는 무조건 silent mutation 이 아니에요. 브라우저 승인, OS 설치,
  원격 repo/app 생성, dependency install 은 consent 가 필요해요. 대신 consent 뒤 흐름을 onboarding 이 이어가요.
...
init/deploy 전에 1회 완료한다는 뜻 — auth 직후. 그래서 onboarding 은 `needs-preflight: false` 유지
...
대화가 끊겨도 사용자가 "승인했어", "다시 온보딩해줘", "온보딩 계속" 이라고 말하면 같은 감지 루프가
다음 gap 부터 이어가야 해요.
...
- **R1 (의존성 게이트 flip, 中~高):** `allows-dependency-execution: true` 는 임의 코드 실행 벡터를
  ...
  + 테스트 동반 갱신 필요.
- **R2 (god-skill, 中):** phase 12개로 onboarding 비대. 완화: (a) 전부 위임 — onboarding 은 순서 + git/node +
...
- **R5 (100% ready 과장, 中):** 브라우저 OAuth, GitHub App 설치, OS installer GUI 는 agent 가 대신 클릭할
  ...
  즉, 외부 행동이 필요해도 onboarding 이 정확한 카드와 재개 phrase 를 주고, 사용자가 돌아오면 같은 루프가
...
| 파일 | 변경 책임 |
|------|-----------|
| `skills/onboarding/SKILL.md` | 온보딩 단일 진입점, state machine, phase/gap table, git/node/GitHub/deps/doctor/final ready card |
| `tests/fixtures/ask-defaults/registry.json` | onboarding 신규 AUQ safe default + rationale |
| `scripts/skill-doctor-allowlist.json` | `onboarding` dep-exec allowlist rationale 추가 |
| `scripts/skill-doctor.ts` | 필요 시 system-install guard/문구 검사 표면 추가 |
| `tests/skill-doctor-dep-execution.test.ts` | onboarding dep-exec allowlist + `--ignore-scripts` 회귀 |
| `tests/skill-noninteractive-guard.test.ts` | 신규 AUQ D1 guard 문구 회귀 |
| `tests/ux-todowrite.test.ts` | expanded checklist stale 방지 회귀 |
| `tests/manifest.test.ts` | onboarding first-run contract, github 라우팅, ready card 문구 회귀 |
...
> onboarding 진화이므로 `skill:new` scaffold 아님 — `skills/onboarding/SKILL.md` 직접 편집하되 Phase 17/18 패턴
> (D1 guard / TodoWrite Step 0 / AUQ registry / 해요체 / keyword baseline) 전부 유지.

- [ ] **T0 (P0, 제품 계약 잠금)** — `skills/onboarding/SKILL.md` 에 **VIBE_READY contract** 추가:
...
- [ ] **T1 (P1)** — onboarding/SKILL.md: 버전체크 phase — CLI(`update`) Phase A apply, 플러그인(`upgrade`)
...
- [ ] **T7 (P1, 게이트변경+보안)** — 의존성 설치 gate flip **전체 표면** (Phase C, repo on disk 뒤 — codex #3):
  ...
  onboarding 추가** + `tests/skill-doctor-dep-execution.test.ts` 갱신 (미등록 시 `skill:doctor` exit 1 = CI 즉사),
...
- [ ] **T9 (P3)** — `docs/HOOKS.md`/관련 docs 에 phase 변경 반영, onboarding description 트리거 어구 유지
  (변경 금지 — baseline lock).
...
- [ ] **T11 (P2, 측정)** — `tests/vibe-bootstrap-measurement.test.ts` 또는 baseline measurement 에
  ...
  Verify: step count/major context-switch count 가 기존 이전 온보딩 흐름 대비 감소.

## 8. Acceptance Criteria (100% onboarding 기준)

- [ ] `온보딩`, `처음인데 뭐부터`, `getting started` 가 모두 `onboarding` 으로 라우팅돼요.
- [ ] 사용자는 sibling skill 이름이나 slash command 를 몰라도 온보딩을 끝낼 수 있어요.
- [ ] onboarding 은 항상 detect-first → 첫 gap 처리 → 재감지 루프를 타요. 한 번에 여러 mutate gap 을 추측해
  실행하지 않아요.
...
대부분 task 가 `onboarding/SKILL.md` 단일 파일을 만져 **순차 구현이 기본**이에요. 분리 가능한 lane:

| Lane | Tasks | 모듈 | 비고 |
|------|-------|------|------|
| A (주) | T0→T1→T2→T3→T4→T6 | `skills/onboarding/SKILL.md` | 같은 파일 — 순차 |
...
⚠️ **거짓 병렬 정정 (codex #7):** T7 의 허용목록 rationale + 안전 테스트는 A 가 onboarding/SKILL.md 에 박는
