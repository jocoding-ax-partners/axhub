...
| ax-hub-cli compatibility | `>=0.1.0,<0.13.0` |
...
| Korean trigger lexicon | 18 skills, 200+ trigger phrases |
| License | MIT |
...
**Q: ax-hub-cli 새 버전이 나오면 어떻게 되나요?**
A: 플러그인이 semver gate 갖고 있어요. 현재 검증 범위는 `<0.13.0` 이고, 범위를 넘는 CLI 가 감지되면 `/axhub:update` 또는 플러그인 업그레이드로 안내해요.
