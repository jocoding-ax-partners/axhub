...
**상태**: 7 SKILL (onboarding · init · deploy · development · diagnosis · clarity · update) · ax-hub-cli v0.20.0+ 직접 호출
...
플러그인은 7개 스킬을 담아요. `onboarding`·`init`·`deploy` 세 핵심 플로우, 기존 앱에 실데이터(connector·table) 기반 기능 코드를 만드는 `development`, 배포 실패 원인을 읽기 전용으로 요약하는 `diagnosis`, CLI·플러그인을 지금 최신으로 올리는 `update`, 그리고 나머지 스킬에 명확히 안 맞거나 의도가 불분명한 axhub 요청을 라이브로 찾아 처리하는 `clarity` 브리지예요.
...
`clarity` 브리지는 정해진 명령 목록을 들고 있지 않아요. `axhub --help` 트리를 라이브로 탐색해 맞는 명령을 찾고 바로 실행해요 — CLI 가 새 명령을 추가해도 플러그인 수정 없이 따라가요. 단, 배포 실패 원인을 명시적으로 묻는 요청은 `diagnosis` 가 맡고 raw 로그 대신 다섯 가지 결과로 요약해요.
...
명령어를 외울 필요 없어요. 평소 말투로 말하면 7개 스킬 중 맞는 곳으로 연결되고, 나머지 스킬 범위 밖이거나 의도가 모호하면 `clarity` 브리지가 axhub 명령을 직접 찾아 실행해요.
...
4개 레이어로 보면: **① 사용자(한국어)** → **② Claude Code (7 skills)** → **③ ax-hub-cli (axhub 바이너리 — plugin-support 그룹 + 공개 표면)** → **④ axhub-api backend**.
...
| 문서 | 용도 |
|------|------|
| [CLAUDE.md](CLAUDE.md) · [AGENTS.md](AGENTS.md) | AI 에이전트 작업 규칙 — 7 skill 체제 · CLI 호출 표면 · release 계약 |
