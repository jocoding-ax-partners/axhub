...
**상태**: 10 SKILL (onboarding · bootstrap · scaffold · publishing · deploy · import · development · diagnosis · clarity · update) · ax-hub-cli 직접 호출 (코어 v0.20.0+ · 기존 8스킬 v0.21.3+ · scaffold v0.30.0+ · publishing은 `axhub plugin publish` 기능 게이트)
...
> axhub CLI가 없거나 필요한 기능이 없으면 관련 스킬이 멈추고 설치/업그레이드를 안내해요 — 최소 요구 버전은 코어 **v0.20.0**, 기존 8스킬 전체는 **v0.21.3**, `scaffold`는 **v0.30.0**이고, `publishing`은 `axhub plugin publish --help` 성공을 기능 게이트로 확인해요.
...
- **Claude Code** 최신 버전
- **axhub CLI v0.20.0 이상 (기존 8스킬은 v0.21.3 이상, scaffold는 v0.30.0 이상)** — `publishing`은 별도 version 추측 대신 `axhub plugin publish --help`가 성공할 때만 진행해요. 기능이 없으면 `update`로 보내고 우회하지 않아요.
...
플러그인은 10개 스킬을 담아요. `onboarding`·`bootstrap`·`scaffold`·`deploy` 네 앱 플로우, plugin·skill을 tenant marketplace에 게시하는 `publishing`, 기존 로컬 앱을 가져오는 `import`, 실데이터 기능을 만드는 `development`, 배포 실패를 읽기 전용으로 진단하는 `diagnosis`, CLI·플러그인을 최신화하는 `update`, 그리고 명시적 운영 명령을 라이브로 찾는 `clarity` 브리지예요.
...
| `publishing` | plugin·skill marketplace 게시 | "플러그인 올려줘", "여러 스킬을 하나로 퍼블리시해줘" |
...
명령어를 외울 필요 없어요. 평소 말투로 말하면 10개 스킬 중 맞는 곳으로 연결돼요. plugin·skill 게시에는 `publishing`, CLI·플러그인 최신 확인에는 `update`, 로그·환경변수·롤백·테이블·GitHub 재연결 같은 운영 명령에는 `clarity` 브리지가 대응해요.
...
- **Plugin·skill 게시하기** — "이 SKILL.md를 우리 회사 마켓에 올려줘", "여러 스킬을 하나의 plugin으로 퍼블리시해줘"
