...
**상태**: 5 SKILL (onboarding · init · deploy · development · clarity) · ax-hub-cli v0.20.0+ 직접 호출
...
플러그인은 6개 스킬을 담아요. `onboarding`·`init`·`deploy` 세 핵심 플로우, 기존 앱에 실데이터(connector·table) 기반 기능 코드를 만드는 `development`, CLI·플러그인을 지금 최신으로 올리는 `update`, 그리고 그 다섯에 명확히 안 맞거나 의도가 불분명한 axhub 요청을 라이브로 찾아 처리하는 `clarity` 브리지예요.
...
명령어를 외울 필요 없어요. 평소 말투로 말하면 6개 스킬 중 맞는 곳으로 연결되고, 나머지 스킬 범위 밖이거나 의도가 모호하면 `clarity` 브리지가 axhub 명령을 직접 찾아 실행해요.
...
4개 레이어로 보면: **① 사용자(한국어)** → **② Claude Code (6 skills)** → **③ ax-hub-cli (axhub 바이너리 — plugin-support 그룹 + 공개 표면)** → **④ axhub-api backend**.
...
| 문서 | 용도 |
|------|------|
| [CLAUDE.md](CLAUDE.md) · [AGENTS.md](AGENTS.md) | AI 에이전트 작업 규칙 — 6 skill 체제 · CLI 호출 표면 · release 계약 |
