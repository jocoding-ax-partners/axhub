...
플러그인에는 42개 SKILL 이 들어 있어요. 아래처럼 **그냥 평소 말투로 말하면** 알아서 맞는 기능을 찾아 실행해요. 명령어를 외울 필요 없어요. 자주 쓰는 슬래시 명령은 옆에 같이 적어뒀어요.
...
| "결과 봐", "앱 열어줘" | 배포가 끝난 앱을 브라우저로 바로 열어줘요 | — |
| "방금 거 안 돼, 살려줘" | 배포가 실패했을 때 원인을 찾아 복구를 도와줘요 | — |
| "이전 배포로 되돌려줘" | 잘 돌던 예전 버전으로 되돌려요. 새 배포가 망가졌을 때 써요 | — |
| "왜 실패했는지 추적해줘" | 배포가 어디서 왜 깨졌는지 단계별로 짚어줘요 | — |
| "진짜 배포됐는지 확인해줘" | 실제 서버 상태를 직접 확인해 배포가 제대로 됐는지 알려줘요 | — |
| "앱 복제해줘", "잠깐 멈춰줘", "다시 켜줘" | 앱을 복사하거나, 잠시 멈췄다가 다시 켜요 | — |

### 📱 앱 만들고 가져오기

| 이렇게 말하면 | 이런 일을 해줘요 | 슬래시 |
|---------------|------------------|--------|
| "결제 앱 만들어줘" | 새 앱을 처음부터 만들어요. 뼈대를 잡아주니 바로 코딩을 시작해요 | — |
| "기존 앱 올려줘" | 이미 있는 내 코드(repo)를 axhub 앱으로 옮겨 올려요 | — |
| "GitHub repo 연결해" | GitHub 저장소를 연결해서, push 만 하면 자동으로 배포되게 해요 | — |
| "내 앱 목록 보여줘" | 내가 가진 앱들을 보여주고, 작업할 앱을 고르게 해요 | `/axhub:apps` |
| "다른 앱 둘러봐", "템플릿 뭐 있어?" | 공개된 앱이나 시작용 템플릿을 구경해요 | — |
| "앱 마켓에 공개해줘" | 내가 만든 앱을 마켓플레이스에 올려 공개 심사에 제출해요 | — |

### 🗄️ 데이터 다루기

| 이렇게 말하면 | 이런 일을 해줘요 | 슬래시 |
|---------------|------------------|--------|
| "데이터 카탈로그 검색해줘" | 앱에서 쓸 수 있는 데이터가 뭐가 있는지 찾아줘요 | — |
| "내가 쓸 수 있는 리소스 보여줘" | 내 권한으로 접근 가능한 데이터·리소스를 보여줘요 | — |
| "리소스 정리해줘", "이름 바꿔줘" | 데이터 리소스를 이름변경·이동·태그·정리로 깔끔하게 관리해요 | — |
| "DB 연결해줘", "postgres 붙여줘" | 외부 데이터베이스(postgres/mysql 등)를 연결하고 관리해요 | — |
| "테이블 만들어줘", "컬럼 추가해줘" | 앱에서 쓰는 표(테이블)를 만들고 컬럼·권한·데이터를 관리해요 | — |

### ✅ 코드 품질 챙기기

| 이렇게 말하면 | 이런 일을 해줘요 | 슬래시 |
|---------------|------------------|--------|
| "코드 리뷰해줘" | 짜놓은 코드를 살펴보고 문제될 만한 부분을 짚어줘요 | — |
| "이거 디버그해줘" | 안 돌아가는 코드를 같이 들여다보며 원인을 찾아요 | — |
| "왜 자꾸 실패하지?" | 배포·테스트가 실패하면 자동으로 원인을 진단해줘요 | — |
| "TDD 로 짜줘" | 테스트를 먼저 만들고 그걸 통과하게 구현하는 방식으로 도와줘요 | — |
| "개발 계획 세워줘" | 무엇부터 어떻게 만들지 단계별 계획을 짜줘요 | — |
| "리뷰 통과했으니 배포해" | 품질 점검까지 끝난 코드를 곧장 배포로 이어줘요 | — |
| (자동으로 동작) | 품질 자동모드가 뭔지·언제 켜지는지 안내해줘요 | — |
| (자동으로 동작) | 좋은 코딩 습관 가이드를 참고해 조언해줘요 | — |

### 🔧 셋업하고 점검하기

| 이렇게 말하면 | 이런 일을 해줘요 | 슬래시 |
|---------------|------------------|--------|
| "처음인데 셋업해줘" | 로그인·CLI 설치·앱 연결까지 처음 쓰는 데 필요한 걸 한 번에 잡아줘요 | — |
| "axhub 로그인해줘" | axhub 에 로그인해요 (브라우저로 승인) | `/axhub:login` |
| "axhub CLI 설치해줘" | axhub 명령줄 도구(CLI)를 설치해요 | — |
| "CLI 최신 버전으로 올려줘" | axhub CLI 를 최신 버전으로 업데이트해요 | `/axhub:update` |
| "플러그인 업그레이드해줘" | 이 플러그인 자체를 새 버전으로 올려요 | — |
| "axhub 잘 설치됐어?" | 설치·로그인·환경이 정상인지 한 번에 진단해요 | `/axhub:doctor` |
| "설정 확인해줘", "axhub.yaml 맞아?" | 앱 설정 파일(axhub.yaml)과 현재 설정·상태가 올바른지 검증해요 | — |
| "환경변수 뭐 있어?" | 쓸 수 있는 환경변수를 보여주고 설정해요 | — |
| "회사 서버 주소 바꿔줘" | 접속할 endpoint(회사 서버) 같은 프로필을 바꿔요 | — |
| "상태줄 켜줘" | 화면 아래 상태줄(statusline)을 켜서 현재 상태를 늘 보이게 해요 | — |
| "라우팅 통계 보여줘" | 내 말이 어떤 기능으로 연결됐는지 통계를 보여줘요 | — |

### 👥 워크스페이스와 팀

| 이렇게 말하면 | 이런 일을 해줘요 | 슬래시 |
|---------------|------------------|--------|
| "내 워크스페이스 보여줘", "테넌트 목록" | 내가 속한 워크스페이스·테넌트 목록과 소속을 보여줘요 | — |
| "팀원 초대해줘", "권한 줘" | 워크스페이스나 앱에 팀원을 초대하고 접근 권한을 관리해요 | — |
