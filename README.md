...
| `list_deployments` | 직접 HTTP 클라이언트 + TLS pinning |
...
| consent 토큰 (세션별) | `$XDG_RUNTIME_DIR/axhub/consent-<session_id>.json` |
...
- 키는 `$XDG_STATE_HOME/axhub/hmac-key`(32바이트, 최초 생성), 토큰은 `$XDG_RUNTIME_DIR/axhub/consent-<session>.json`, 기본 TTL 60초.
...
ls "${XDG_RUNTIME_DIR:-/tmp}/axhub/"                            # consent-*.json
...
- **TLS pinning** — `axhub-api.jocodingax.ai` SPKI 핀(`list_deployments.rs`). 핀 불일치 → `security.tls_pin_failed`. self-host 호스트는 스킵, `AXHUB_ALLOW_PROXY=1` 이면 우회
...
| `AXHUB_ENDPOINT` | axhub-api endpoint override |
| `AXHUB_ALLOW_PROXY=1` | TLS pinning 우회 (프록시 환경) |
...
| `list_deployments` | 직접 HTTP 클라이언트 + TLS pinning (`verify_hub_api_tls_pin`) |
...
| 크레이트 | 쓰임 |
|----------|------|
| `reqwest` + `rustls`(ring) + `webpki-roots` + `x509-parser` | HTTP 클라이언트 + TLS handshake + SPKI 핀 추출 |
