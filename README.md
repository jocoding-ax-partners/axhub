...
플러그인은 4개 스킬만 담아요. `onboarding`·`init`·`deploy` 세 핵심 플로우와, 그 셋에 명확히 안 맞거나 의도가 불분명한 axhub 요청을 라이브로 찾아 처리하는 `clarity` 브리지예요.
...
`clarity` 브리지는 정해진 명령 목록을 들고 있지 않아요. `axhub --help` 트리를 라이브로 탐색해 맞는 명령을 찾고 바로 실행해요 — CLI 가 새 명령을 추가해도 플러그인 수정 없이 따라가요.

---

## 💬 자연어로 할 수 있는 일

명령어를 외울 필요 없어요. 평소 말투로 말하면 4개 스킬 중 맞는 곳으로 연결되고, 핵심 3스킬 범위 밖이거나 의도가 모호하면 `clarity` 브리지가 axhub 명령을 직접 찾아 실행해요.
