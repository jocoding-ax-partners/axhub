...
**상태**: 7 SKILL (onboarding · init · deploy · import · development · clarity · update) · ax-hub-cli v0.20.0+ 직접 호출
...
플러그인은 7개 스킬을 담아요. `onboarding`·`init`·`deploy` 세 핵심 플로우, 비어 있지 않은 기존 로컬 앱을 axhub로 가져오는 `import`, 기존 앱에 실데이터(connector·table) 기반 기능 코드를 만드는 `development`, CLI·플러그인을 지금 최신으로 올리는 `update`, 그리고 그 여섯에 명확히 안 맞거나 의도가 불분명한 axhub 요청을 라이브로 찾아 처리하는 `clarity` 브리지예요.
