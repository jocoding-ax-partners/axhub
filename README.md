...
[![version](https://img.shields.io/badge/version-0.10.5-blue)](https://github.com/jocoding-ax-partners/axhub/releases)
...
**상태**: v0.10.5 · 4 SKILL (onboarding · init · deploy · cli) · ax-hub-cli v0.20.0+ 직접 호출
...
- **Claude Code** 최신 버전
- **axhub CLI v0.20.0 이상** — `init`·`deploy` 스킬이 시작 시 버전을 확인해요. 미설치 시 `onboarding` 스킬이 설치를 안내해요.
...
플러그인은 4개 스킬만 담아요. `onboarding`·`init`·`deploy` 세 핵심 플로우와, 그 범위 밖 모든 axhub 작업을 라이브로 찾아 실행하는 `cli` 브리지예요.
...
| `cli` | 그 외 모든 작업 | "환경변수 설정해줘", "로그 보여줘", "롤백해줘", "앱 목록 보여줘" |

`cli` 브리지는 정해진 명령 목록을 들고 있지 않아요. `axhub --help` 트리를 라이브로 탐색해 맞는 명령을 찾고 바로 실행해요 — CLI 가 새 명령을 추가해도 플러그인 수정 없이 따라가요.

---

## 💬 자연어로 할 수 있는 일

명령어를 외울 필요 없어요. 평소 말투로 말하면 4개 스킬 중 맞는 곳으로 연결되고, 핵심 3스킬 범위 밖이면 `cli` 브리지가 axhub 명령을 직접 찾아 실행해요.
...
- CLI 가 새 기능을 내면 자연어 트리거만 더하면 돼요 — `cli` 브리지는 그것조차 자동이에요.
...
4개 레이어로 보면: **① 사용자(한국어)** → **② Claude Code (4 skills)** → **③ ax-hub-cli (axhub 바이너리 — plugin-support 그룹 + 공개 표면)** → **④ axhub-api backend**.
...
- **최소 버전 게이트** — `init`·`deploy` 스킬은 시작 시 `axhub --version` 으로 최소 버전(v0.20.0)을 확인하고, 없거나 낮으면 멈추고 설치/업그레이드를 안내해요 — 우회하지 않아요.
...
| 문서 | 용도 |
|------|------|
| [CLAUDE.md](CLAUDE.md) · [AGENTS.md](AGENTS.md) | AI 에이전트 작업 규칙 — 4 skill 체제 · CLI 호출 표면 · release 계약 |

살아남은 quality gate 는 셋이에요.

```bash
bun run lint:tone --strict   # 모든 한글 텍스트 해요체 0 err
bun test                     # SKILL frontmatter 유효성 + e2e flow
...
> 판정·실행 로직은 플러그인이 아니라 `ax-hub-cli` 에 있어요 — helper 기능 변경은 그쪽 레포(`ax-hub-cli`)에서 해요.
