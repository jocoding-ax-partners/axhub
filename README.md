...
대표 성공 여정은 **첫 셋업 → 앱 생성 → 배포 → 상태 확인**이에요. 각 단계는 `onboarding` 이 CLI·로그인·환경을 detect-first 로 확인하고, `init` 이 앱 생성과 첫 배포를 이어가며, `deploy` 가 preview-confirm 뒤 `axhub deploy verify <deployment-id> --app <app>` 로 성공을 확정하고, 이후 상태·로그·환경변수 같은 나머지 작업은 `clarity` 가 공개 CLI 표면에서 찾아 처리해요.
