...
description: '이 스킬은 사용자가 axhub statusline/상태바를 활성화하고 싶을 때 사용해요. 다음 표현에서 활성화: "상태바 켜줘", "statusline 켜줘", "status line 활성화", "statusline 활성화", "statusline 보여줘", "enable statusline", "activate statusline", "상태표시줄 켜줘", "상태줄 활성화". Claude Desktop 일반 발화는 안전한 요약 helper 한 번으로 처리하고, 기존 다른 상태바가 있으면 덮어쓰지 않아요.'
examples:
  - utterance: "상태바 켜줘"
    intent: "enable axhub status bar safely"
...
For ordinary Claude Desktop prompts like `상태바 켜줘`, `상태표시줄 켜줘`, `statusline 켜줘`, or `상태줄 활성화`, use this path and stop after it.

1. The first visible chat sentence must be exactly `상태바 설정을 확인할게요.` with no planning sentence before or after it.
2. Use exactly one Bash tool call.
3. For Bash tool calls, set the tool `description` or title exactly `상태바 설정`.
4. Run exactly:

   ```bash
   axhub-helpers statusline-summary --user-utterance "<latest user sentence>"
   ```

5. Copy the Korean stdout as the answer and do not reinterpret it.

Natural-language summary contract:
- Preserve an existing third-party status bar by default.
- Do not run `settings-merge`, `autowire-statusline`, file reads, snippet printing, clipboard commands, or restart commands directly for an ordinary Desktop prompt.
- Do not mention slash commands, skill names, route names, helper names, raw command names, raw settings JSON, existing command strings, exit codes, scope fallback narration, `statusLine`, `wire`, `settings-merge`, file paths, or English tool-title fragments.
- Do not show the current contents of any Claude settings file.
- If the helper says another status bar is already enabled, say only that axhub preserved it and ask the user to say they want to switch if they do.
...
2. **선택지별 처리.**
   ...
   **`자동으로 켜요` 선택 시:**
   ...
   - 4 (PreservedOther): 다른 plugin 의 statusLine 발견 — preserve 했어요. 강제 override 는 "statusline 다시 켜줘"라고 말하면 재실행
   ...
   **`나중에 할래요` 선택 시:**
   ...
   조용히 exit 0 해요. 나중에 "statusline 켜줘"라고 말하면 다시 불러요.
