...
- **언제:** 통합 승인에서 실행 가능한 옵션을 고른 직후, execute 호출 직전에만 해요. `manifest_create` 가 없으면(=axhub.yaml 이 이미 있으면) 절대 건드리지 않아요. headless 에서는 실행하지 않아요.
- **무시 파일 선행 정리:** axhub.yaml 을 쓰기 전에 반드시 먼저 tool cwd 를 `APP_DIR` 로 지정하고 bare `git check-ignore -q axhub.yaml` 로 ignore 여부를 확인해요. 현재 Desktop 프로젝트가 `APP_DIR` 이면 `cd`, `&&`, `;`, `echo $?`를 붙이지 않아요. 무시 중이면 `.gitignore` 또는 `.git/info/exclude` 에서 `axhub.yaml` 을 직접 무시하는 줄을 제거하고, 다시 `git check-ignore -q axhub.yaml` 이 실패하는지 확인해요. 아직 무시되면 axhub.yaml 을 쓰거나 `axhub deploy --explain --json` 검증을 진행하지 말고 남은 ignore 출처를 정리해요. 이 변경은 앱 설정을 첫 배포에 반영하기 위한 manifest 보강의 일부이므로, `--commit-manifest` 경로를 선택했다면 `.gitignore` 변경도 같은 커밋에 들어가야 해요.
...
- **무시 파일 사후 확인:** 검증 통과 직후, execute 전에 `git check-ignore -q axhub.yaml` 이 실패하는지 다시 확인해요. 아직 무시되면 execute 를 호출하지 말고 남은 ignore 출처를 정리해요. 실패한 execute 뒤에 뒤늦게 고치는 복구 흐름으로 두지 않아요.
- **첫 배포까지 반영 (옵션, commit+push):** `axhub plugin-support preflight` 의 `capabilities.import.commit_manifest` 가 true 이고 GitHub 기반 첫 배포(docker/compose, 또는 preview 가 `github_repo_create`/`github_connect`/`first_deploy` 를 요구하는 경우)라면 preview 통합 승인에서만 선택해요. **local_only 앱은 아직 git remote 가 없을 수 있지만, execute 중 CLI 가 repo/remote 를 만들 수 있으므로 remote 없음만으로 이 선택지를 빼지 않아요.** `설정도 반영하고 시작` 을 골랐으면 검증 뒤 다시 묻지 않고 execute 를 `--commit-manifest` 로 호출해요. `커밋 없이 시작` 이면 옵션을 빼요. 강제 push 는 절대 안 하고, headless 에서는 제공하지 않아요.
