...
1. **엔진과 현재 커넥터를 확인해요.**
   ...
   axhub connectors list --tenant "$TENANT" --enabled-only --json
   axhub connectors discover "$CONNECTOR_ID" --tenant "$TENANT" --json
   ```
...
4. **mutation 명령.**
   ...
   axhub connectors create --tenant "$TENANT" --name "$NAME" --engine postgres --config-file cfg.json --credentials-file creds.json --execute --json
   axhub connectors update "$CONNECTOR_ID" --tenant "$TENANT" --config-file cfg.json --enabled --execute --json
   axhub connectors credentials-set "$CONNECTOR_ID" --tenant "$TENANT" --credentials-file creds.json --execute --json
   axhub connectors delete "$CONNECTOR_ID" --tenant "$TENANT" --execute --json
   ```
