...
`1` CLI 가드 → `1.1` deploy-prep(resolve+preflight) → `0` TodoWrite(1.1 결과로 도출) → `1.1b` first-run 브리지 → `1.5` git 저장 준비 → `1.6` in-flight 감지 → `1.7` status-first 게이트 → `2` preview 결정(headless) → `3` preview 카드 → `3.5` 토큰 게이트 → `4` deploy create(fallback) → `5` verify(성공 선언) → `5b` 업데이트 알림 → `6` 비-0 에러 라우팅 / `7` dry-run 트리거.
...
**Routing 게이트 (Step 1 직후, resolve 전에 실행).** 이 SKILL 은 `description:` 의 "배포"·"deploy"·"ship" 어구로도 자동 선택돼서, axhub 와 무관한 프로젝트나 다른 배포 타깃(`vercel` 등)을 쓰려는 발화에도 끌려올 수 있어요. 그래서 **resolve 를 하기 전에** `route-decision` 을 한 번 호출해서 정말 axhub 배포가 맞는지 먼저 확정해요.

`EXPLICIT` 은 호출 모달리티예요. 이 SKILL 이 `/deploy`, `/axhub:deploy`, 또는 `/배포` **슬래시 명령**으로 호출됐으면 `EXPLICIT=1`, 자연어 skill-selection 으로 왔으면 `EXPLICIT=0` 으로 둬요. 확실하지 않으면 `EXPLICIT=1` 로 둬요. `$ARGS` 에는 사용자 발화 원문을 그대로 담아서 `vercel` 같은 타깃 키워드가 살아 있게 해요.
...
`route-decision` 의 binding 계약은 `.decision` 단일이에요 (axhub/yield/ignore/ask). exit 0 고정 fail-open. `ROUTE_DECISION` 값으로 분기해요. **`axhub` 일 때만 진행**해요:

- **`axhub`** → 정상 경로. 아래 Step 1.1 (deploy-prep) 로 계속 진행해요.
- **`yield`** → 사용자가 다른 배포 타깃(예: `vercel`/`netlify`/`fly` 등)을 명시했어요 (named-target-wins). axhub 배포를 멈추고 한 줄로 "다른 배포 타깃을 쓰려는 것 같아서 axhub 배포는 건너뛸게요." 만 안내한 뒤 양보해요. **deploy-prep·`axhub deploy create` 를 하나도 호출하지 말아요.**
- **`ignore`** / **`ask`** → axhub 인지 확실하지 않아요. 아래 AskUserQuestion 으로 한 번 물어봐요. **물어보기 전에는 resolve 를 호출하지 말아요.**
