...
`1` CLI 가드 → `1a` 버전 체크(신버전 안내) → `1.1` deploy-prep(resolve+preflight) → `1.2` static 분기(deploy_method=static 이면 독립 lane) → `0` TodoWrite(1.1 결과로 도출) → `1.1b` first-run 브리지 → `1.5` git 저장 준비 → `1.6` in-flight 감지 → `1.7` status-first 게이트 → `2` preview 결정(headless) → `3` preview 카드 → `3.5` 토큰 게이트 → `4` deploy create(fallback) → `5` verify(성공 선언) → `6` 비-0 에러 라우팅 / `7` dry-run 트리거.
