...
- `auth` / CLI exit 4: subcode `github_relogin_required` 면 GitHub 계정 연동 문제라 device-flow fast path(`axhub github link` 승인 → `axhub github accounts list --json`)로 이어가요 — axhub 재로그인으로는 안 풀려요. 그 외에는 `다시 로그인해줘`.
