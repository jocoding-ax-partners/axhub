...
```bash
# 원격 detect CLI 는 v0.17.3 origin/main 에 없어요.
# helper migrate-plan/local pre-scan 과 axhub.yaml manifest-only preview 를 기본으로 써요.
"$HELPER" migrate-plan --dir "${AXHUB_MIGRATE_DIR:-.}" --json

# 앱 등록. apps create 는 --yes/--execute 없이 직접 mutation 이므로 preview+consent 뒤에만 실행해요.
...
# 실행 전 consent binding 은 action=auth_oauth_client_create, app_id=$APP_ID,
...
`axhub apps detect` 는 v0.17.3 origin/main 에 없는 branch-only future 계약이에요. 현재 스킬 계약은 아래처럼 낮춰요.

- 입력은 로컬 디렉터리만 받아요. `AXHUB_MIGRATE_DIR` 또는 현재 디렉터리를 `--dir` 로 넘기고, monorepo 하위 앱은 `--app-path` 만 추가해요. remote repo/ref/path detect 는 v0.17.3 CLI 에 없어서 raw backend endpoint 로 추측하지 않아요.
...
- exit `0` 은 preview 성공이에요. exit `64` 는 local path 형식·필수 플래그 같은 helper validation 실패예요. exit `1` 은 local generic 실패로 다뤄요.
- non-zero local scan 은 앱 등록·git 연결·배포를 시작하지 않고, helper error envelope 의 request_id/subcode/doc_url 이 있으면 그대로 보여줘요.

remote detect CLI 는 현재 쓰지 않아요. v0.17.3 에서 `axhub apps detect --help` 는 Unknown 경로라 raw backend endpoint 를 직접 curl 하지 않고 helper/local manifest-only 로 멈춰요.
...
4. **기존 mutation 경로 재사용.** 앱 등록, git 연결, env 값 저장, 배포는 위 CLI boundary contract 와 기존 consent 경로만 써요. helper 로 consent 를 우회하지 않아요. `axhub apps git connect --execute` 는 `github_connect` consent 승인 카드가 있어야만 실행해요. remote detect 는 v0.17.3 에서 current command 가 아니에요. local dir 은 helper pre-scan 과 manifest 초안까지만 처리하고, raw backend 호출 없이 다음 작업을 안내해요.
...
6. **선택형 Auth Migration.** 사용자가 원할 때만 기존 앱의 로그인 흐름을 AxHub OAuth/OIDC 로 바꾸는 절차를 진행해요. 기본 migrate 는 배포 완료가 끝이에요. 이 단계는 사용자 앱 auth 파일 변경을 다루는 절차이고, AxHub backend/gateway 코드나 Data Gateway/DB query refactor 는 범위가 아니에요.
   ...
   OAuth client 는 raw backend endpoint 를 curl 하지 않고 axhub CLI 로만 만들어요. 실행 전 `axhub-helpers consent-mint` 로 `action=auth_oauth_client_create`, top-level `app_id`, `context={name,type,redirect_uris,scopes,grant_types}` 를 mint 해요. 아래 예시는 `context.redirect_uris="$CALLBACK_URL"`, `context.scopes="openid,profile,email"`, `context.grant_types="authorization_code"` 와 정확히 맞아야 해요.

   ```bash
   cat <<JSON | "$HELPER" consent-mint
   {"tool_call_id":"pending","action":"auth_oauth_client_create","app_id":"${APP_ID}","profile":"","branch":"","commit_sha":"","context":{"name":"web","type":"confidential","redirect_uris":"${CALLBACK_URL}","scopes":"openid,profile,email","grant_types":"authorization_code"}}
   JSON
