...
- `reviewer` stage 는 전용 task agent 가 없으므로 **read-only `executor` lane** 으로 completeness / scope sanity check 를 실행하고 `.axhub/plan/runs/<run_id>/stages/05-reviewer.md` + meta 저장
- 최종 ADR 저장 → `.axhub/plan/runs/<run_id>/adr.md`
- `approval.json.state=pending_approval`, `run.json.state=pending_approval` 로 올리고 멈춰요

   runtime 에서는 `task` tool 로 bundled agent 를 명시해요. 즉 planner stage 는 `planner`, architect stage 는 `architect`, critic stage 는 `critic`, reviewer stage 는 read-only `executor` lane 이에요. architect 와 critic 은 한 parallel batch 로 묶지 말고 항상 architect 완료 후 critic 을 실행해요.
