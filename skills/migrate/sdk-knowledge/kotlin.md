...
# AxHub kotlin SDK — migrate knowledge pack

This pack is the load-bearing context for `axhub-sdk-kotlin-expert`. The **Client
...
fun main() = runBlocking {
    val tenantId = System.getenv("AXHUB_TENANT_ID")
    val tenantSlug = System.getenv("AXHUB_TENANT_SLUG")
    val client = AxHubKotlinClient(
        baseUrl = "https://api.axhub.ai",
        token = System.getenv("AXHUB_TOKEN"),
        tokenType = TokenType.PAT,
    ...
    val me = client.request("authGetApiV1Me")
    val userId = me["userId"] as? String
        ?: ((me["user"] as? Map<*, *>)?.get("id") as? String)
        ?: error("authGetApiV1Me did not return a user id")

    val suffix = System.currentTimeMillis().toString()
    val slug = "agent-kt-" + suffix.takeLast(8)
    val table = "items" + suffix.takeLast(6)

    val app = client.apps.create(mapOf(
        "slug" to slug,
        "name" to "Agent Kotlin README QA",
        "visibility" to "private",
        "auth_mode" to "anonymous",
        "resource_tier" to "S",
        "deploy_method" to "docker",
        "subdomain" to slug,
    ))
    val appId = app["id"] as String

    client.data.schemaPostApiV1AppsByAppIDTables(
        pathParams = mapOf("appID" to appId),
        body = mapOf(
            "table_name" to table,
            "owner_column" to "owner_id",
            "columns" to listOf(
                mapOf("name" to "owner
...
- Route inventory: `client.routes`, `client.contextRoutes`, `client.errorCodes`.
- Streaming route inventory for agents: `client.routeFlow()`.
- Errors: catch `AxHubException` from the Java SDK and branch on `code()`, `category()`, `status()`, and `retryable()`.
...
_The kotlin SDK has NO ergonomic data layer yet — it is a route-table passthrough (data access = raw `method(path_params, query, body)` against the route surface in §4). Reliable ORM→SDK data conversion is NOT available for kotlin until the ergonomic fluent data layer ships (tracked: the SDK expansion program). For kotlin, `data_patch_plan` stays **PLAN-ONLY**: describe the intended change in the Korean preview, do NOT emit data-call code. Data route identities for reference:_
