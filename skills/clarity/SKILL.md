...
- 브라우저가 성공 화면인데 CLI background task 가 화면에서 오래 남아 있으면 그 상태만 기다리지 말고 `인증 확인` 제목으로 `axhub github accounts list --tenant <tenant> --json` 을 한 번 조회해서 계정 연결 반영 여부를 확인해요. 계정 목록이 정상으로 오면 인증은 완료된 것으로 요약하고, 대기 중인 프로세스가 있으면 정리해요.
