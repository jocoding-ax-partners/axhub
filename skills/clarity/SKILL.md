---
name: clarity
description: 'This skill resolves any axhub-related request that does NOT clearly belong to the onboarding, init, or deploy skills — including vague or ambiguous ones — by discovering the right axhub CLI command and running it. 이 스킬은 axhub 관련 요청 중 온보딩·새 앱 만들기·배포에 명확히 속하지 않거나 의도가 불분명한 발화를 받아, axhub CLI 의 `--help` 트리를 탐색해 알맞은 명령을 찾고 에이전트가 알아서 실행해요. 다음 표현에서 활성화: "환경변수 설정해줘", "env 설정", "시크릿 추가해줘", "로그 보여줘", "앱 목록 보여줘", "앱 지워줘", "상태 확인해줘", "롤백해줘", "이전 버전으로 되돌려줘", "테넌트 바꿔줘", "팀 전환해줘", "멤버 초대해줘", "비용 보여줘", "프로필 바꿔줘", "업데이트 확인해줘", "axhub로 ~해줘", 그리고 "axhub로 뭔가 해줘", "이거 axhub에서 어떻게 해?" 처럼 axhub 관련이지만 무엇을 원하는지 명확하지 않은 모든 발화. 배포 실행·검증은 deploy, 새 앱 생성은 init, 처음 셋업은 onboarding 이 담당하니 그 의도가 분명하면 그쪽으로 보내요.'
---
...
5. **결과 제시.** exit 0 이면 무엇이 어떻게 됐는지 한국어 한두 문장으로 요약해요 (URL·이름 같은 사용자 가치 정보만, 내부 id·raw JSON 생략). 비-0 이면:
   ...
   - 사용법 오류(exit 64) → 탐색 2단계로 돌아가 인자를 고쳐 1회 재시도, 그래도 실패하면 정직하게 설명.
