...
새 axhub 앱을 current CLI template registry 에서 시작해요. v0.2.0 에서는 dependency install 과 remote template fetch 를 하지 않고 `axhub --json init --list-templates` 를 source of truth 로 써요. Sprint 3 부터 scaffold 뒤에는 helper bootstrap 을 plan-only 로만 호출해서 다음 안전 단계를 보여줘요.
...
5. **Bootstrap plan-only 로 다음 안전 단계를 보여줘요.** scaffold 가 성공한 뒤에는 원격 앱 생성이나 배포를 실행하지 말고, Rust bootstrap FSM 을 읽기/계획 모드로만 호출해요.

   ```bash
   ${CLAUDE_PLUGIN_ROOT}/bin/axhub-helpers bootstrap --dry-run --json
   ```

   `consent_required_apps_create`, `git_init_required`, `first_commit_required`, `template_required`, `conflict_existing_files` 같은 상태를 그대로 보여주고, 앱 등록이나 배포는 deploy/apps 흐름으로 이어가요. init 은 파일 생성까지만 맡고, `bootstrap --auto-chain`, `apps create`, `deploy create` 는 실행하지 않아요.
