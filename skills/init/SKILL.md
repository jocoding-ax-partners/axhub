...
Clone 이 성공하면 먼저 `axhub.yaml` 의 앱 바인딩을 `$APP_SLUG` 와 맞춰요. 현재 manifest 에서는 최상위 `name:` 이 app slug 로 resolve 돼요. 템플릿 기본값(예: `name: vite-react-axhub`)이 남으면 나중에 deploy resolve 가 다른 앱을 찾거나 app 을 못 찾아 잘못된 UX 로 빠질 수 있어요.
