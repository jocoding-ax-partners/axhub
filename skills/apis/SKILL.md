...
examples:
  - utterance: "내 apis catalog 알려줘"
    intent: "browse axhub api catalog"
  - utterance: "사내 API 뭐 있어"
    intent: "browse axhub api catalog"
  - utterance: "list apis"
    intent: "browse axhub api catalog"
  - utterance: "available endpoints"
    intent: "browse axhub api catalog"
  - utterance: "사용 가능한 api"
    intent: "browse axhub api catalog"
multi-step: false
...
4. **Cross-team opt-in.** If the user explicitly says "다른 팀 API도 보고싶어", "회사 전체 API", "all apis", or asks after seeing the scoped list, surface AskUserQuestion:
   ...
   ```json
   {
     "question": "다른 팀 API도 볼래요? 권한 있는 모든 API 주소를 보여줄 수 있지만, 보통 현재 앱이 호출하는 것만 봐도 충분해요.",
     "header": "팀 범위",
     "options": [
