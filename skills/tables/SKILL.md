...
앱 동적 테이블의 schema, grants, row 데이터를 관리해요. destructive DDL/DML 은 dry-run preview 와 명시 확인 뒤에만 `--execute` 해요.
...
- 사용자에게 route label, slash command, `workflow`, `워크플로`, skill 이름, `preflight`, approval context, command name, raw command line, raw question JSON, raw JSON, raw email, raw id, raw app slug, local file contents, repo inspection, 영어 tool title fragment, A/B 구현 라벨을 쓰지 않아요.
...
- create + column 같은 복합 요청은 먼저 대상 app/table/columns preview 를 보여주고, 동의가 확인된 뒤 승인된 `--execute` 명령을 하나만 실행해요.
...
5. **Run exactly one mutation command after approval.** 여러 컬럼·행·권한 변경은 각 명령마다 preview + approval + one execute command 를 반복해요.
