...
3. **읽기 명령을 실행해요.**
   ...
   ```bash
   axhub invitations list --status pending --expires-within 168h --tenant "$(jq -r '.tenant // empty' .axhub/state/tenant.json 2>/dev/null)" --json
   axhub members list --tenant "$(jq -r '.tenant // empty' .axhub/state/tenant.json 2>/dev/null)" --json
   axhub members me --tenant "$(jq -r '.tenant // empty' .axhub/state/tenant.json 2>/dev/null)" --json
...
4. **mutation 은 preview 후 실행해요.**
   ...
   ```bash
   axhub invitations send "$EMAIL" --role member --tenant "$(jq -r '.tenant // empty' .axhub/state/tenant.json 2>/dev/null)" --json
   axhub invitations bulk --from-file users.csv --role member --strict --execute --tenant "$(jq -r '.tenant // empty' .axhub/state/tenant.json 2>/dev/null)" --json
   axhub invitations cancel "$INVITE_ID" --execute --tenant "$(jq -r '.tenant // empty' .axhub/state/tenant.json 2>/dev/null)" --json
   axhub invitations resend "$INVITE_ID" --role member --execute --tenant "$(jq -r '.tenant // empty' .axhub/state/tenant.json 2>/dev/null)" --json
