...
1. **전체 상태 감지 (read-only, 위임 전).** preflight 를 먼저 부르지 말아요 — CLI 가 아직 없을 수 있어요.
   ...
   ```powershell
   # CLI — 부재 가능
   if (Get-Command axhub -ErrorAction SilentlyContinue) { axhub --version } else { "cli-missing" }
   # node — 머신 레벨 존재 확인 (없으면 Step 4 에서 설치)
   if (Get-Command node -ErrorAction SilentlyContinue) { node --version } else { "node-missing" }
   # node 가 있을 때만 부가 advisory: pm 선호(lockfile) + 권장 버전(.nvmrc/engines)
   Get-ChildItem bun.lockb,pnpm-lock.yaml,package-lock.json,yarn.lock -ErrorAction SilentlyContinue
   if (Test-Path .nvmrc) { Get-Content .nvmrc }
   node -p "require('./package.json').engines?.node" 2>$null
   ```
   ...
   "$HELPER" preflight --json   # auth_ok, user_email
   ```

   Windows PowerShell:

   ```powershell
   & "$env:CLAUDE_PLUGIN_ROOT\bin\axhub-helpers.exe" preflight --json   # auth_ok, user_email
   ```

   `CLAUDE_PLUGIN_ROOT` 가 비어있으면 `Get-Command axhub-helpers` 또는 cache 경로(`$env:USERPROFILE\.claude\plugins\cache\axhub\axhub\*\bin\axhub-helpers.exe`)의 최신 버전을 써요 (doctor 패턴 — awk/sort 등가물 대신 산문 폴백).

   CLI 와 로그인이 둘 다 ✓ 면 GitHub App 설치 상태도 read-only 로 확인해요 (로그인 안 됐으면 건너뛰고 "로그인 후 확인" 으로 표시해요). 이건 차단 단계가 아니라 항상 보여주는 안내예요 — 이미 설치했거나 나중에 할 사람은 그냥 넘어가도 돼요 (OS 공통).

   ```bash
   axhub github accounts list --json   # data.accounts[]: login, installed, install_url
   ```

   `data.accounts[]` 를 `installed:true` / `installed:false` 로 나눠요. `install_url` 은 아무 entry 에서나 읽으면 돼요 (전부 같아요). 목록이 비어 있으면 (아직 아무 계정에도 설치 안 됨) install_url 을 억지로 만들지 말고 첫 앱 만들 때 GitHub 연결 안내가 자동으로 뜬다고만 알려줘요. `installation_id` 같은 internal 값은 echo 하지 말고, `login` 과 `install_url` 만 보여줘요.
...
2. **온보딩 체크리스트 카드.** 현 상태를 ✓/✗ 로 보여주고 첫 번째 gap 을 안내해요 (해요체).
   ...
   axhub 온보딩 상태:
     ...
     · GitHub App: 설치된 계정 <N>개 (또는 아직 없음 / 로그인 후 확인)
   ```

   GitHub App 줄은 `·` 로 표시해요 — ✗ 라도 흐름을 막지 않는 안내라서 첫 번째 gap 으로 위임하지 않아요. CLI/로그인/node 만 gap 으로 처리해요.
...
4. **node 런타임 consent-gate 설치.** silent 설치는 안 해요 — consent 한 번 받고 진행해요. node 는 axhub 와 달리 공식 `curl|bash` installer 가 없어서, 최선 방법 fallback chain 으로 설치해요.
   ...
   ```json
   {
     "questions": [{
       ...
       "multiSelect": false,
       "options": [
         {"label": "지금 설치", "description": "있는 패키지 매니저로 설치 (Unix 는 없으면 nvm, Windows 는 수동 안내)"},
   ...
   "지금 설치" 선택 시 fallback chain (consent 받았으니 실행):
   ...
   - **1순위 — 이미 있는 패키지 매니저** (가장 깨끗): node 는 axhub CLI 와 달리 OAuth client_id 발급이 없어서, install-cli 가 pm 채널을 피하는 이유(pm 갱신 지연 → 구버전 client_id → 로그인 실패)가 node 에는 해당 안 돼요. 그래서 node 는 pm 설치가 안전해요.
     ...
     - Windows (PowerShell): `winget install OpenJS.NodeJS.LTS` 또는 `scoop install nodejs-lts`
     - Linux: `apt-get install -y nodejs` / `dnf install -y nodejs` / `pacman -S nodejs` (배포판별)
   - **2순위 (Unix: macOS/Linux 한정) — 패키지 매니저 없음 → nvm 설치 스크립트** (⚠️ supply-chain deviation, 아래 NEVER 예외 참고):
     ```bash
     curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.1/install.sh | bash
     # shell rc 재로드 후
     nvm install --lts
     ```
     nvm 버전 태그(`v0.40.1`)는 핀 고정해요 — 임의 latest 금지. 도메인은 `raw.githubusercontent.com/nvm-sh` 한정. **Windows 는 이 자동 2순위가 없어요** — nvm-windows 는 `.exe` GUI installer 라 비대화형 설치에 안 맞아서, winget/scoop 이 안 되면 바로 3순위(수동)로 가요.
   - **3순위 — 다 안 되면** → https://nodejs.org LTS 다운로드를 안내만 해요 (Windows 는 winget/scoop 실패 시 여기로).

   설치 후 `node --version` 으로 재확인해요 (명령은 OS 공통).
...
5. **준비/handoff 카드.** doctor 스타일 ✓/✗ 로 마무리해요.
   ...
   axhub 셋업 결과:
     ...
     · GitHub App: <설치된 계정 목록> (또는 아직 없음)
   이제 배포할 준비 끝났어요!
   ```

   **GitHub App 설치 안내 (항상 보여주고, 안 해도 돼요).** Step 1 의 `axhub github accounts list --json` 결과로 아래 셋 중 하나를 보여줘요. 링크는 보여주기만 하고 자동으로 열지 않아요 — 이미 됐거나 나중에 할 사람은 그냥 넘어가도 돼요.

   - 설치된 계정 ≥1: "GitHub App 이 설치된 계정: `<login...>`. 다른 org/계정을 더 붙이려면 이 링크로 설치해요: `<install_url>`"
   - 미설치 계정만/혼재: "GitHub App 이 안 깔린 계정: `<login...>`. 설치 링크: `<install_url>`"
   - 빈 목록: "아직 GitHub App 이 설치된 계정이 없어요. 첫 앱을 만들면 GitHub 연결 안내가 자동으로 떠요."
...
6. **첫 앱 연결.** 모두 ✓ 이고 앱 manifest 가 없으면 첫 앱 만들기를 제안해요. manifest 는 canonical `axhub.yaml` 을 우선 확인하고, `apphub.yaml` 도 인정해요.

   Unix / Git Bash:

   ```bash
   test -f axhub.yaml || test -f apphub.yaml || echo "no-manifest"
   ```

   Windows PowerShell:

   ```powershell
   if (-not ((Test-Path axhub.yaml) -or (Test-Path apphub.yaml))) { "no-manifest" }
   ```

   `no-manifest` 면 첫 앱 만들기를 제안해요:
