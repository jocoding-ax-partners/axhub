...
6. **첫 앱 연결.** 모두 ✓ 이고 앱 manifest 가 없으면 첫 앱 만들기를 제안해요. manifest 는 canonical `apphub.yaml` 을 우선 확인하고, legacy `axhub.yaml` 도 보조로 인정해요.

   Unix / Git Bash:

   ```bash
   test -f apphub.yaml || test -f axhub.yaml || echo "no-manifest"
   ```

   Windows PowerShell:

   ```powershell
   if (-not ((Test-Path apphub.yaml) -or (Test-Path axhub.yaml))) { "no-manifest" }
   ```

   legacy `axhub.yaml` 만 있으면 "`mv axhub.yaml apphub.yaml` 로 최신 이름으로 바꾸면 좋아요" 한 줄을 곁들여요. `no-manifest` 면 첫 앱 만들기를 제안해요:
