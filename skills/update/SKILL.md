---
name: update
description: '이 스킬은 사용자가 axhub CLI 업데이트를 확인하거나 적용하고 싶어할 때 사용합니다. 다음 표현에서 활성화: "버전 확인", "새 버전", "새 버전 나왔어", "새 버전 받아", "업그레이드", "업그레이드해", "업데이트", "업데이트 있어", "업데이트 적용해주세요", "업데이트 확인해", "업데이트해", "최신", "최신 버전으로 올려주세요", "최신으로 올려", "최신이야", "axhub 새 버전 있어", "axhub 업그레이드 부탁드려요", "brew upgrade 해줘", "CLI 업데이트 부탁드려요", "brew upgrade", "check version", "latest", "new release", "update", "update available", "upgrade", "version", 또는 axhub CLI 버전 관리 요청. 체크 전용 자연어에서는 helper, doctor, preflight 로 우회하지 않고 CLI update check 만 실행합니다.'
examples:
...
Check and apply axhub CLI updates through the axhub CLI's real update contract.

## Claude Desktop natural-language path

For ordinary Claude Desktop update-check prompts such as `업데이트 필요한지 봐줘`, `업데이트 있어?`, `새 버전 나왔어?`, or `최신인지 봐줘`, do **not** expose raw update JSON to the user.

1. First visible sentence exactly:

   ```
   업데이트를 확인할게요.
   ```

2. Use exactly one Bash tool call with title/description exactly:
...
3. Run exactly:
   ...
   ```bash
   axhub-helpers update-summary --user-utterance "<latest user sentence>"
   ```

   The helper runs the real read-only `axhub update check --json` command and converts only `current`, `latest`, and update availability into Korean user-facing text.

4. Copy the Korean stdout as the answer and stop. Do not add another table or diagnosis.

5. If stdout says an update is available, do not run `axhub update apply` yet. Ask the user to say `업데이트 적용해줘`; the apply path still needs preview plus explicit approval.

Do not show route labels, slash commands, skill names, raw command names, raw JSON field names such as `has_update`, filesystem paths, installer URLs, plugin update suggestions, preflight/auth narration, cache scans, compatibility diagnostics, or English tool-title fragments.

## Critical execution contract

For non-Desktop/raw CLI update/version checks, including scripted QA or evidence-mode prompts, the first read-only CLI command is:

```bash
axhub update check --json
```

Do not run `preflight`, `doctor`, `auth status`, plugin manifest inspection, helper discovery, cache scanning, or compatibility diagnostics before this command. Those are different workflows. If the user also asks for plugin install health or compatibility, finish the CLI update check first, then suggest the natural follow-up phrase "설치 상태 진단해줘". In Claude Desktop natural-language mode, use the `update-summary` helper above instead; it exists only to prevent raw JSON leakage while preserving the same read-only CLI check.

Do not mention or invent update-related environment-variable toggles. The CLI owns signature enforcement internally; the skill should describe that as "기본 서명 검증" only.
...
1. **Check for update.** Run `axhub update check --json` as the first and only read-only check command:

   ```bash
   axhub update check --json
   ```

   이 스킬은 `needs-preflight: false` 예요. 버전 확인 요청에서는 `axhub-helpers`, `axhub auth status`, `axhub auth refresh`, `axhub auth login`, preflight, doctor, helper/cache discovery, plugin manifest inspection, 또는 환경 진단으로 우회하지 말고 `axhub update check --json` 를 첫 read-only 명령으로 정확히 한 번 실행해요. 응답을 파싱해요: `{"current": "v0.17.4", "latest": "v0.18.0", "has_update": true}` 또는 `{"has_update": false}`. `has_update: false` 면 이미 최신이에요.
...
4. **On `apply`.** Preview the apply plan first:
   ...
   **On "적용":** cosign 은 기본 enforce 라 env 접두 없이 실행해요:
   ...
   CLI 가 swap 전에 기본 서명 검증을 수행해요. exit 0 (`{"applied":true, "install_kind":"self_replace", "current":..., "latest":..., "binary":"<path>"}`) 이면: "업그레이드 완료! v<LATEST> 깔렸어요 (`<binary>`). 새 터미널을 열거나 `axhub --version` 으로 확인하면 돼요." 라고 안내해요.
...
6. **exit 15 (swap failed).** 원자적 바이너리 교체가 실패했어요. **자동 재시도하지 말아요** (바이너리가 부분 교체됐을 수 있어요). `~/.axhub/bin/axhub.<old>.bak` 백업이 있으면 복구에 쓸 수 있어요. "설치 상태 진단해줘"라고 말하면 이어서 점검할 수 있다고 안내해요.
...
7. **exit 66 — `error.subcode` 로 분기해요.**
   ...
   - `update.cosign_enforce_failed`: 서명 검증 실패예요. **하드 스톱**, 어떤 우회도 제시하지 말아요. `../deploy/references/error-empathy-catalog.md` 의 서명 검증 항목으로 라우팅하고 안내해요:
...
- 서명 검증 모드 (CLI 기본 서명 검증 — 별도 env 토글 없음)
...
- NEVER apply 호출에 env 접두를 붙이거나 env 토글을 안내하지 않아요. 서명 검증은 CLI 기본 정책이라 별도 env 가 필요 없어요.
- NEVER `--force` 를 서명 검증 우회로 제시 — `--force` 는 다운그레이드 게이트만 풀고 서명 검증은 절대 안 풀어요.
- NEVER update check 요청에서 helper, cache scan, plugin manifest inspection, auth/preflight/doctor 흐름으로 우회하지 않아요. 이 스킬의 첫 명령은 `axhub update check --json` 이에요.
...
exit 66 signature / digest 템플릿: `../deploy/references/error-empathy-catalog.md`.
