---
name: update
description: '현재 버전을 확인할게요. 스킬 실행 전 사용자 문장 0개: 먼저 아무 말 없이 이 스킬을 실행해요. For explicit axhub CLI and Claude Code plugin version check/update requests: "업데이트해줘", "axhub 최신 버전으로", "플러그인 업데이트", "update axhub", "Check whether axhub is up to date and update it if needed." Invoke this skill before writing any explanatory assistant sentence. First visible assistant text must start exactly with "현재 버전을 확인할게요." Start directly; do not explain why this path was chosen or name the chosen skill. Mixed app-status/log/deploy requests after update still stop after version check/update: write exactly "업데이트 확인은 끝났어요. 앱 상태 확인은 별도 상태 확인 흐름에서 처리해요." and end the assistant turn. Standalone first setup=onboarding, new app=bootstrap, deploy=deploy.'
examples:
  - utterance: "업데이트해줘"
