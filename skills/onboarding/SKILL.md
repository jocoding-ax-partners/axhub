...
- **먼저 권하지는 않아요.** GitHub 연동에는 push 자동 배포·저장소·이력·협업이 따라오니 기본 안내는 그대로예요. 이 문장은 gate 에서 멈출 때 붙이는 **대안**이에요.
- 실제 실행은 앱을 만드는 일이라 bootstrap 스킬의 12단계가 담당해요 — onboarding 은 길이 있다는 것만 알려주고 넘겨요.
